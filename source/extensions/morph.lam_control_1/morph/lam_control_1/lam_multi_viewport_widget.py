...
- ``LAM_SimSplit_*`` Workspace 창 / Dock / ``create_viewport_window`` **절대 사용 안 함**
...
_SPLIT_FRAME_SLOT = "morph.lam_control_1:00_split_viewport_widgets"
_VP_TILE_MIN_PX = 64
...
def sim_viewport_split_widget_enabled() -> bool:
    try:
        from .lam_sim_control_defaults import USE_VIEWPORT_WIDGET_SPLIT
        ...
        pass
    try:
        v = str(os.environ.get("LAM_SIM_VIEWPORT_WIDGET_SPLIT", "") or "").strip().lower()
...
def is_split_widget_layout_active(ext: Any) -> bool:
    return bool(getattr(ext, "_lam_split_used_widget_layout", False))


def _tile_usd_context_name(tile_index: int) -> str:
    return "" if int(tile_index) <= 0 else f"morph_lam_split_aux_{int(tile_index)}"


def _tile_win_name(tile_index: int) -> str:
    return "Viewport" if int(tile_index) <= 0 else f"LAM_SimSplit_{int(tile_index)}"
...
def _log_widget_lifecycle(ext: Any, phase: str, wn: str, rec: Optional[Dict[str, Any]]) -> None:
    ...
    if not isinstance(rec, dict):
        try:
            print(f"[LAM widget-life] {phase} tile={wn!r} rec=None", flush=True)
        except Exception:
            pass
        return
    ...
    try:
        print(
            f"[LAM widget-life] {phase} tile={wn!r} "
            ...
            f"shell_hstack={_lifecycle_id(getattr(ext, '_lam_widget_shell_hstack', None))} "
            f"create_count={int(getattr(ext, '_lam_widget_create_total', 0) or 0)}",
            flush=True,
        )
    except Exception:
        pass
...
def _refresh_rec_api_from_widget(rec: Dict[str, Any], wn: str) -> Any:
    ...
    if widget is None:
        ...
        try:
            print(
                f"[LAM widget-life] api=None tile={wn!r} — widget already destroyed (no recreate)",
                flush=True,
            )
        except Exception:
            pass
    ...
    try:
        print(
            f"[LAM widget-life] api=None tile={wn!r} widget={_lifecycle_id(widget)} still alive",
            flush=True,
        )
    except Exception:
        pass
...
def _destroy_all_aux_workspace_windows(ext: Any = None) -> None:
    """
    Dock 모드 잔여 ``LAM_SimSplit_*`` Workspace 창 제거.

    Widget 분할 모드에서는 ``LAM_SimSplit_1`` 이 **논리 타일명**이기도 하다.
    ``get_viewport_from_window_name('LAM_SimSplit_1')`` 이 embedded ``ViewportWidget`` API 를
        ...
        if ext is not None and not widget_mode:
            try:
                for ent in list(getattr(ext, "_lam_multi_viewport_entries", []) or []):
                    ...
                    if not wn.startswith("LAM_SimSplit"):
                        continue
...
def _remove_aux_fallback_dome_light(stage: Any, *, label: str = "") -> bool:
    ...
    Returns:
      삭제/비활성 시도가 있었으면 True. 로그: [LAM/p0b-rig-clone] remove_fallback
    """
    if stage is None:
        return False
    ...
    try:
        ...
        print(
            f"[LAM/p0b-rig-clone] remove_fallback label={label!r} "
...
def _sync_aux_stage_lighting_from_main(aux_ctx_name: str) -> int:
    ...
    if not aux_ctx_name:
        return 0
    try:
        from .lam_viewport_p0b_diag import log_p0b_call_order, p0b_disable_clone
        ...
        pass
    try:
        from .lam_sim_control_defaults import VIEWPORT_AUX_LIGHTING_SYNC_FROM_MAIN
    ...
    try:
        ...
        try:
            print(
                f"[LAM multi-sim] aux 조명 동기화 실패 ctx={aux_ctx_name!r}: {exc}",
                flush=True,
            )
        except Exception:
            pass
    ...
    if copied > 0:
        try:
            print(
                f"[LAM multi-sim] aux 조명 동기화 완료 ctx={aux_ctx_name!r} "
            ...
            pass
    try:
        from .lam_viewport_p0b_diag import log_p0b_call_order
...
def _ensure_aux_stage_default_lighting(ctx_name: str) -> None:
    ...
    주의 (P0-B):
      ...
      로그: `[LAM multi-sim] 보조 스테이지 fallback DomeLight 추가`
    ...
    if not ctx_name:
        return
    try:
        from .lam_viewport_p0b_diag import (
            _env_flag,
            log_p0b_call_order,
    ...
    try:
        ...
        if stage.GetPrimAtPath(light_path).IsValid():
            return
        try:
            from .lam_viewport_p0b_diag import log_fallback_create_stack
        ...
        try:
            print(
                f"[LAM multi-sim] 보조 스테이지 fallback DomeLight 추가 ctx={ctx_name!r}",
                flush=True,
            )
        except Exception:
            pass
        ...
        try:
            print(
                f"[LAM multi-sim] 보조 스테이지 조명 추가 실패 ctx={ctx_name!r}: {exc}",
                flush=True,
            )
        except Exception:
            pass
...
def _clone_default_light_rig_from_main_to_aux(ext: Any) -> int:
    ...
    try:
        ...
        if p0b_disable_clone():
            ...
            try:
                print("[LAM/p0b-rig-clone] skip: TBS_P0B_DISABLE_CLONE=1", flush=True)
            except Exception:
                pass
            return 0
    ...
    try:
        if not is_split_widget_layout_active(ext):
            try:
                print("[LAM/p0b-rig-clone] skip: split_widget_layout_inactive", flush=True)
            except Exception:
                pass
            return 0
    except Exception as exc:
        try:
            print(f"[LAM/p0b-rig-clone] skip: layout_check_err={exc}", flush=True)
        except Exception:
            pass
        return 0
    ...
    if main_ctx is None or aux_ctx_obj is None:
        try:
            print(
                f"[LAM/p0b-rig-clone] skip: ctx_none main={main_ctx is None} aux={aux_ctx_obj is None}",
                flush=True,
            )
        except Exception:
            pass
    ...
    if main_stage is None or aux_stage is None:
        try:
            print(
                f"[LAM/p0b-rig-clone] skip: stage_none main={main_stage is None} aux={aux_stage is None}",
                flush=True,
            )
        except Exception:
            pass
    ...
    if not need:
        try:
            print(
                f"[LAM/p0b-rig-clone] skip: need=False main_lux={main_lux!r} aux_lux={aux_lux!r}",
                flush=True,
            )
        except Exception:
            pass
    ...
    try:
        ...
        if aux_sess is None:
            try:
                print("[LAM/p0b-rig-clone] skip: aux_session=None", flush=True)
            except Exception:
                pass
            return 0
        ...
        try:
            print(
                f"[LAM/p0b-rig-clone] src_layer={getattr(src_layer,'identifier',None)!r}",
                flush=True,
            )
        except Exception:
            pass
        ...
        if not any(p.startswith("/OmniKit_Viewport_LightRig/") for p in aux_check):
            ...
            try:
                print(
                    f"[LAM/p0b-rig-clone] composed_author n={authored} "
        ...
        if has_rig_mid:
            ...
            try:
                print(
                    "[LAM/p0b-rig-clone] no_lightrig_after_clone: "
        ...
        if not ok:
            try:
                print(
                    f"[LAM/p0b-rig-clone] FAILED copied={copied} "
            ...
            try:
                from .lam_viewport_p0b_diag import p0b_log

                p0b_log(
                    f"[LAM/p0b-rig-clone] copied={copied} ok={ok} "
                    f"has_rig={has_rig} has_fallback={has_fallback} "
        ...
        try:
            print(f"[LAM/p0b-rig-clone] err={exc}", flush=True)
        except Exception:
            pass
        return 0
...
async def connect_widget_tile_main_stage(ext: Any, token: int = 0) -> bool:
    ...
    if not is_split_widget_layout_active(ext):
        return False
    if bool(getattr(ext, "_lam_main_stage_connected", False)):
        return True
    tiles = getattr(ext, "_lam_split_widget_tiles", None)
    ...
    try:
        ext._lam_main_stage_connected = True
    except Exception:
        pass

    _log_widget_lifecycle(ext, "connect-main-after", "Viewport", main_rec)
    try:
        print("[LAM multi-sim] 화면1 Stage 연결 (Widget 재생성 없음)", flush=True)
    except Exception:
        pass
    return True
...
async def assign_widget_split_cameras(
    ...
    _activate_tile_manipulator_only(
        ext, str(getattr(ext, "_lam_active_widget_tile", "") or "Viewport")
    )
...
def _disable_viewport_window_camera_bindings(ext: Any) -> None:
    ...
    if not is_split_widget_layout_active(ext):
        return
    try:
        from .lam_sim_control_defaults import VIEWPORT_DISABLE_NATIVE_CAMERA_BINDINGS

        if not bool(VIEWPORT_DISABLE_NATIVE_CAMERA_BINDINGS):
            return
    except Exception:
        pass
    if bool(getattr(ext, "_lam_camera_bindings_disabled", False)):
        return
    try:
        import carb.settings

        settings = carb.settings.get_settings()
        if not hasattr(ext, "_lam_camera_bindings_saved"):
            try:
                ext._lam_camera_bindings_saved = settings.get(_CAM_BINDINGS_CARB_KEY)
            except Exception:
                ext._lam_camera_bindings_saved = None
        settings.set(_CAM_BINDINGS_CARB_KEY, {})
        ext._lam_camera_bindings_disabled = True
        try:
            print(
                "[LAM multi-sim] ViewportWindow camera bindings disabled "
        ...
        try:
            print(f"[LAM multi-sim] camera bindings disable fail: {exc}", flush=True)
        except Exception:
            pass
...
def _activate_tile_manipulator_only(ext: Any, win_name: str) -> None:
    ...
    try:
        ext._lam_active_widget_tile = wn
    except Exception:
        pass
    tiles = getattr(ext, "_lam_split_widget_tiles", None)
    if not isinstance(tiles, dict):
    ...
    _disable_native_viewport_navigation_permanent(ext)
    try:
        from .lam_viewport_coupling_diag import log_manipulator_activation_state
...
def _enforce_widget_tile_manipulator_isolation(ext: Any) -> None:
    ...
    active = str(getattr(ext, "_lam_active_widget_tile", "") or "Viewport")
...
class _WidgetHudMount:
    """타일 HUD 용 ``get_frame`` + ``viewport_api`` (FOUP/라벨 SceneView mount)."""

    def __init__(self, overlay: Any, viewport_api: Any = None) -> None:
...
def _resolve_main_viewport_window(ext: Any) -> Any:
    ...
    try:
        vw = getattr(ext, "_lam_split_main_viewport_window", None)
...
def _suspend_native_viewport_widget_presenter(ext: Any) -> None:
    ...
    if not is_split_widget_layout_active(ext):
        return
    try:
        vw_host = getattr(ext, "_lam_split_main_viewport_window", None) or _resolve_main_viewport_window(
            ext
        )
        if vw_host is None:
            return
        ...
        try:
            ...
            pass
        saved = getattr(ext, "_lam_native_vp_presenter_state", None)
        if not isinstance(saved, dict):
            saved = {}
            try:
                ext._lam_native_vp_presenter_state = saved
            except Exception:
                pass
...
def _restore_native_viewport_widget_presenter(ext: Any) -> None:
    ...
    if not isinstance(saved, dict):
        return
    try:
        ext._lam_native_vp_presenter_state = {}
    except Exception:
        pass
...
def _suspend_orphan_viewport_manipulators(ext: Any) -> None:
    ...
    try:
        ...
        from .lam_multi_viewport import _set_model_navigation_enabled
...
def _collect_camera_manipulator_models_for_window_from_api(api: Any) -> List[Any]:
    ...
    if api is None:
        return models
    try:
        from .lam_multi_viewport import _is_viewport_camera_manipulator_model
    except Exception:
        return models
    for attr in (
        "camera_manipulator",
...
def _suspend_native_viewport_manipulators(ext: Any = None) -> None:
    ...
    try:
        ...
        # Widget 모드: ``LAM_SimSplit_*`` 는 논리 타일명 — embedded API 와 충돌하므로 제외.
        win_names = ["Viewport"]
        try:
            if ext is None:
...
def _stop_native_viewport_input_guard(ext: Any) -> None:
    sub = getattr(ext, "_lam_widget_native_guard_sub", None)
    if sub is not None:
        try:
            sub.unsubscribe()
        except Exception:
            pass
    try:
        ext._lam_widget_native_guard_sub = None
    except Exception:
        pass
...
def _start_native_viewport_input_guard(ext: Any, token: int) -> None:
    ...
    try:
        ext._lam_widget_native_guard_sub = (
            kit_app.get_app()
            .get_post_update_event_stream()
            .create_subscription_to_pop(_tick, name="morph.lam_control_1.widget_native_guard")
        )
    except Exception:
        pass
...
def _set_native_viewport_input_blocked(ext: Any, blocked: bool) -> None:
    ...
    try:
        ...
        if api is None:
            try:
                ext._lam_widget_native_vp_suspended = bool(blocked)
            except Exception:
                pass
            return
        if blocked:
...
def _set_native_viewport_updates_enabled(ext: Any, enabled: bool) -> None:
    ...
    try:
        ...
        if not enabled:
            ...
            _suspend_native_viewport_manipulators(ext)
        try:
            ext._lam_widget_native_vp_suspended = not bool(enabled)
        except Exception:
            pass
    except Exception:
        pass
...
def _purge_widget_mode_stale_windows(ext: Any = None) -> None:
    """Widget 분할 — ``LAM_SimSplit_*`` Workspace 창이 있으면 즉시 제거."""
    _destroy_all_aux_workspace_windows(ext)
...
def _create_deferred_aux_viewport_widget(ext: Any, aux_rec: Dict[str, Any]) -> bool:
    ...
    if int(_WIDGET_CREATE_COUNT) != 1:
        try:
            print(
                f"[LAM/hydra-diag] WARN deferred aux create: "
    ...
    zstack = aux_rec.get("_aux_zstack") or getattr(ext, "_lam_widget_shell_zstack_aux", None)
    if zstack is None:
        try:
            print(f"[LAM/hydra-diag] deferred aux create FAIL: no zstack tile={aux_wn!r}", flush=True)
        except Exception:
            pass
        return False
    ...
    try:
        ...
        try:
            print(f"[LAM/hydra-diag] deferred aux ViewportWidget import FAIL: {exc}", flush=True)
        except Exception:
            pass
        return False
    ...
    try:
        print(
            f"[LAM/hydra-diag] deferred aux ViewportWidget create START "
    ...
    try:
        ext._lam_widget_create_total = int(_WIDGET_CREATE_COUNT)
    except Exception:
        pass
    try:
        print(
            f"[LAM/hydra-diag] deferred aux ViewportWidget create DONE "
            f"create_total={_WIDGET_CREATE_COUNT} "
            f"has_render_product={_api_has_render_product(aux_rec.get('api'))} "
            f"hud_mount={aux_rec.get('hud_mount') is not None}",
            flush=True,
        )
    except Exception:
        pass
    # 화면2 타일 준비 직후 — CSV 창 설정(FOUP/기기정보/탑뷰) 동기화
    try:
        ...
        elif win is not None and hasattr(win, "sync_csv_viewport_overlays_for_screen"):
            win.sync_csv_viewport_overlays_for_screen(2)
    except Exception as exc:
        try:
            print(f"[LAM/hydra-diag] deferred aux overlay sync: {exc}", flush=True)
        except Exception:
            pass
...
def _log_hydra_pipeline_diag(
    ...
    *,
    ...
    try:
        print(
            f"[LAM/hydra-diag] phase={phase!r} tile={wn!r} "
        ...
        pass
    try:
        from .lam_viewport_rp_diag import log_rp_investigation, viewport_rp_diag_enabled
...
async def _wait_aux_usd_stage_settled(
    ...
    for frame_i in range(max(4, int(max_frames))):
        ...
        if settle_after_ready >= 6:
            try:
                print(
                    f"[LAM/hydra-diag] aux stage settled ctx={ctx_key!r} "
    ...
    try:
        print(
            f"[LAM/hydra-diag] WARN aux stage settle timeout ctx={ctx_key!r} "
...
def _warn_tile_stage_isolation(ext: Any, rec: Dict[str, Any], *, label: str = "") -> None:
    ...
    try:
        if main_stage is not None and aux_stage is not None and main_stage is aux_stage:
            print(
                f"[LAM multi-sim] WARN stage isolation FAIL tile={wn!r} "
                f"main==aux stage_id={id(main_stage):#x} {label}",
                flush=True,
            )
        elif main_stage is not None and aux_stage is not None:
            print(
                f"[LAM multi-sim] stage isolation OK tile={wn!r} "
...
def _bind_tile_viewport_to_context(
    ...
    *,
    ...
    try:
        ...
        if callable(fn) and resolved_cam is not None and stage is not None:
            if not had_rp_before:
                ...
                try:
                    print(
                        f"[LAM/hydra-diag] bind skip viewport_changed "
    ...
    try:
        print(
            f"[LAM multi-sim] bind ctx tile={wn!r} ctx={ctx_key!r} "
...
def _sync_aux_tile_render_from_main(ext: Any) -> None:
    ...
    try:
        ...
        pass
    if not bool(getattr(ext, "_lam_aux_stage_connected", False)):
        return
    tiles = getattr(ext, "_lam_split_widget_tiles", None)
    if not isinstance(tiles, dict):
        return
...
def _ensure_tile_manipulator(ext: Any, win_name: str, rec: Dict[str, Any]) -> bool:
    ...
    if manip is None:
        try:
            print(
                f"[LAM multi-sim] manipulator attach FAIL tile={win_name!r} "
    ...
    try:
        from .lam_viewport_coupling_diag import probe_tile_manipulator

        probe = probe_tile_manipulator(rec, str(win_name))
        print(
            f"[LAM multi-sim] manipulator attached tile={win_name!r} "
...
def _schedule_tile_manipulators_when_ready(ext: Any, token: int, split_n: int) -> None:
        ...
        if remaining[0] <= 0:
            if sub_ref[0] is not None:
                try:
                    ...
                    pass
            tiles_timeout = getattr(ext, "_lam_split_widget_tiles", None)
            if isinstance(tiles_timeout, dict):
                ...
                for wn in ("Viewport",) + tuple(_tile_win_name(ti) for ti in range(1, sn)):
                    ...
                    if isinstance(rec, dict) and rec.get("camera_manipulator") is None:
                        try:
                            print(
                                f"[LAM multi-sim] manipulator TIMEOUT tile={wn!r} "
    ...
    try:
        old = getattr(ext, "_lam_widget_manip_poll_sub", None)
        if old is not None:
            try:
                old.unsubscribe()
            except Exception:
        ...
        sub_ref[0] = kit_app.get_app().get_post_update_event_stream().create_subscription_to_pop(
            _tick,
            name="morph.lam_control_1.widget_manip_ready_poll",
        )
        ext._lam_widget_manip_poll_sub = sub_ref[0]
    except Exception:
        pass
...
def sync_split_widget_fill_frame(ext: Any, split_n: int) -> None:
    ...
    try:
        ...
        if is_lam_hyview_stream_layout_locked(ext):
            bridge_stream_skip(
...
def _log_widget_tile_api(wn: str, ctx_name: str, api: Any) -> None:
    try:
        ...
        print(
            f"[LAM multi-sim] Widget 타일 api tile={wn!r} expect_ctx={ctx_name!r} api_ctx={got!r}",
            flush=True,
        )
    except Exception:
        pass
...
def _destroy_aux_tile_record(rec: Dict[str, Any]) -> None:
    ...
    if backend is not None:
        try:
            from .lam_multi_viewport import _destroy_kit_viewport
...
async def _connect_widget_tile_aux_stage(ext: Any, token: int, sn: int) -> bool:
    ...
    try:
        ...
        pass
    if bool(getattr(ext, "_lam_aux_stage_connected", False)):
        return True
    ...
    try:
        from .lam_viewport_coupling_diag import log_stage_lighting_summary
    ...
    try:
        from .lam_viewport_rp_diag import (
        ...
        if viewport_rp_isolated_test_enabled():
            ...
            try:
                from .lam_viewport_rp_diag import teardown_isolated_rp_test_window
        ...
        try:
            print(f"[LAM/rp-isolated] hook error: {exc}", flush=True)
        except Exception:
            pass
    ...
    try:
        print(
            f"[LAM multi-sim] 화면2 ViewportWidget 최초 생성+Stage 연결 tile={aux_wn!r} ctx={aux_ctx!r}",
            flush=True,
        )
    except Exception:
        pass
    ...
    _set_active_widget_tile(ext, str(getattr(ext, "_lam_active_widget_tile", "") or "Viewport"))

    try:
        ext._lam_aux_stage_connected = True
        ext._lam_aux_vw_materialized = True
    except Exception:
        pass
    ...
    try:
        from .lam_viewport_rp_diag import observe_rp_timeline, viewport_rp_diag_enabled
    ...
    try:
        print(
            "[LAM multi-sim] 화면2 Stage 연결 완료 (단일 Viewport 탭·HStack 50:50)",
            flush=True,
        )
    except Exception:
        pass
...
def _create_viewport_tile(
    ...
    *,
    ...
    try:
        ext._lam_widget_create_total = create_n
    except Exception:
        pass
    try:
        print(
            f"[LAM multi-sim] ViewportWidget #{create_n} 생성 tile={wn!r} ctx={ctx_name!r}",
            flush=True,
        )
    except Exception:
        pass
    ...
    if viewport_api is not None:
        ...
        # Workspace 창명 ``LAM_SimSplit_*`` 와 충돌하지 않는 고유 API id.
        vw_kw["viewport_api"] = f"morph.lam_control_1:widget_tile_{create_n}"
    ...
    try:
        print(
            f"[LAM/hydra-diag] ViewportWidget ctor tile={wn!r} ctx={ctx_name!r} "
    ...
    api = None
    scene_view = None
    hud_overlay: Any = None

    def _build_contents() -> None:
        nonlocal vw_tile, api, scene_view, hud_overlay
        with ui.ZStack():
            ...
            # FOUP/기기/웨이퍼 3D 라벨용 overlay — ViewportWidget 과 같은 ZStack 최상단
            # (deferred aux 에서도 ui_container 안에 있어야 get_frame mount 가 보임)
    ...
    try:
        ...
        try:
            print(
                f"[LAM multi-sim] ViewportWidget 실패 tile={wn!r} ctx={ctx_name!r}: {exc}",
                flush=True,
            )
        except Exception:
            pass
    ...
    if hud_overlay is None:
        # fallback — 타일 ZStack 밖이면 라벨이 안 보이므로 경고만
        try:
            print(
                f"[LAM multi-sim] WARN tile={wn!r} hud_overlay missing — "
                "FOUP/device labels may not mount",
                flush=True,
            )
        except Exception:
            pass
        ...
        with hud_overlay:
            pass
    hud_mount = _WidgetHudMount(hud_overlay, viewport_api=api)
...
def _build_split_widget_shell(
    ...
    *,
    ...
    try:
        ext._lam_widget_shell_hstack = shell_hstack
        ext._lam_widget_shell_zstack_main = z1
        ext._lam_widget_shell_zstack_aux = z2
        ext._lam_widget_create_total = int(_WIDGET_CREATE_COUNT)
    except Exception:
        pass
    ...
    try:
        print(
            "[LAM multi-sim] ViewportWidget shell — 화면1=#1 생성, "
...
async def _materialize_aux_viewport_widget(ext: Any, token: int, sn: int) -> bool:
    ...
    if bool(getattr(ext, "_lam_aux_stage_connected", False)):
        return True
    tiles_done = getattr(ext, "_lam_split_widget_tiles", None)
    aux_rec = (
    ...
    if isinstance(aux_rec, dict) and aux_rec.get("widget") is not None and bool(
        ...
        return True
    if bool(getattr(ext, "_lam_aux_vw_materialize_inflight", False)):
        for _ in range(48):
            if int(getattr(ext, "_lam_multi_viewport_apply_token", 0) or 0) != token:
                return False
            if bool(getattr(ext, "_lam_aux_stage_connected", False)):
                return True
            await kit_app.get_app().next_update_async()
        return bool(getattr(ext, "_lam_aux_stage_connected", False))
    if not _aux_stage_ready(ext):
        return False
    if int(getattr(ext, "_lam_multi_viewport_apply_token", 0) or 0) != token:
        return False
    ...
    try:
        ext._lam_aux_vw_materialize_inflight = True
    except Exception:
        pass
    try:
        return await _connect_widget_tile_aux_stage(ext, token, sn)
        ...
        try:
            ext._lam_aux_vw_materialize_inflight = False
        except Exception:
            pass
...
def request_aux_widget_materialize(ext: Any, token: int, sn: int) -> None:
    ...
    if bool(getattr(ext, "_lam_aux_stage_connected", False)):
        return
    try:
        from .lam_multi_viewport import startup_dual_orchestration_active
...
def _destroy_split_widget_host_ui(ext: Any) -> None:
    try:
        from .lam_viewport_coupling_diag import teardown_camera_change_tracker
    ...
    if isinstance(tiles, dict):
        for rec in tiles.values():
            ...
            rec["viewport_window"] = None
    try:
        ext._lam_split_widget_tiles = {}
    except Exception:
        pass
    vw = getattr(ext, "_lam_split_viewport_host_window", None)
    if vw is None:
    ...
    try:
        ext._lam_aux_cell_frame = None
        ext._lam_widget_shell_hstack = None
        ext._lam_widget_shell_zstack_main = None
        ext._lam_widget_shell_zstack_aux = None
        ext._lam_split_viewport_host_window = None
        ext._lam_split_widget_host_win = None
    except Exception:
        pass
    _set_native_viewport_updates_enabled(ext, True)
    _stop_native_viewport_input_guard(ext)
    sub = getattr(ext, "_lam_widget_nav_hold_sub", None)
    if sub is not None:
        try:
            sub.unsubscribe()
        except Exception:
            pass
    sub2 = getattr(ext, "_lam_widget_manip_poll_sub", None)
    if sub2 is not None:
        try:
            sub2.unsubscribe()
        except Exception:
            pass
    try:
        ext._lam_widget_nav_hold_sub = None
        ext._lam_widget_manip_poll_sub = None
        ext._lam_widget_stage_refresh_inflight = False
        ext._lam_widget_stage_sync_key = None
        ext._lam_widget_split_ready = False
        ext._lam_aux_stage_connected = False
        ext._lam_aux_vw_materialized = False
    except Exception:
        pass
...
def teardown_split_widget_host(ext: Any) -> None:
    ...
    try:
        ...
        pass
    try:
        from .lam_multi_viewport import set_viewport_fill_frame_for_split_count
...
def split_widget_layout_healthy(ext: Any, split_n: int) -> bool:
    ...
    if sn < 2:
        return False
    if getattr(ext, "_lam_split_viewport_host_window", None) is None:
        return False
    tiles = getattr(ext, "_lam_split_widget_tiles", None)
    if not isinstance(tiles, dict):
        return False
...
async def _sync_widget_tiles_after_aux_stage_ready(
    ...
    try:
        ext._lam_widget_stage_sync_key = (int(token), int(sn))
    except Exception:
        pass
    try:
        print("[LAM multi-sim] ViewportWidget aux 스테이지 동기화 완료", flush=True)
    except Exception:
        pass
...
async def refresh_split_widget_tiles_after_stage(ext: Any, token: int, n: int) -> None:
    if not is_split_widget_layout_active(ext):
        return
    if bool(getattr(ext, "_lam_widget_stage_refresh_inflight", False)):
        return
    try:
        sn = max(2, int(n))
    except Exception:
    ...
    if getattr(ext, "_lam_widget_stage_sync_key", None) == sync_key:
        return
    try:
        ext._lam_widget_stage_refresh_inflight = True
    except Exception:
        pass
    try:
        tiles = getattr(ext, "_lam_split_widget_tiles", None)
...
def sync_viewport_hud_when_ready(ext: Any, *, force: bool = False) -> None:
    ...
    if not force and bool(getattr(ext, "_lam_ebs_hud_mounted", False)):
        return
    try:
        from .lam_multi_viewport import startup_dual_orchestration_active
        ...
        pass
    try:
        from .lam_multi_viewport import _resolve_viewport_window_for_workspace_name

        vw = _resolve_viewport_window_for_workspace_name("Viewport")
        if vw is not None:
            ext._lam_split_main_viewport_window = vw
    except Exception:
        pass
    if force:
        try:
            ext._lam_ebs_hud_mounted = False
        except Exception:
            pass
    try:
        hud = getattr(ext, "_lam_viewport_control_hud", None)
...
async def finalize_widget_split_startup(ext: Any, token: int, n: int) -> None:
    ...
    if not bool(getattr(ext, "_lam_main_stage_connected", False)):
        try:
    ...
    tiles_now = getattr(ext, "_lam_split_widget_tiles", None)
    aux_wn = _tile_win_name(1)
    aux_rec = tiles_now.get(aux_wn) if isinstance(tiles_now, dict) else None

    try:
        from .lam_viewport_rp_diag import log_finalize_rp_step
        ...
        pass
    try:
        from .lam_multi_viewport import _sync_entries_from_widget_tiles
    ...
    if aux_hydra_ok:
        try:
            print(
                "[LAM/hydra-diag] finalize: aux render_product OK — "
    ...
    try:
        ...
        pass
    _schedule_tile_manipulators_when_ready(ext, int(token), sn)
    tiles_ready = getattr(ext, "_lam_split_widget_tiles", None)
    ...
    tiles_diag = getattr(ext, "_lam_split_widget_tiles", None)
    ...
    try:
        from .lam_viewport_coupling_diag import (
    ...
    try:
        ...
        try:
            print(f"[LAM/p0b-rig-clone] finalize err={exc}", flush=True)
        except Exception:
            pass

    try:
        total = int(getattr(ext, "_lam_widget_create_total", 0) or 0)
        print(
            f"[LAM multi-sim] ViewportWidget 분할 startup READY (create_total={total}, expect=2)",
            flush=True,
        )
    except Exception:
        try:
            print("[LAM multi-sim] ViewportWidget 분할 startup READY", flush=True)
        except Exception:
            pass

    try:
        ext._lam_widget_split_ready = True
    except Exception:
        pass
...
async def apply_split_widget_layout(ext: Any, token: int, n: int) -> bool:
    ...
    try:
        ...
        try:
            print(f"[LAM multi-sim] ViewportWidget import 실패: {exc}", flush=True)
        except Exception:
            pass
        return False
    ...
    if vw_host is None:
        try:
            print("[LAM multi-sim] ViewportWindow(get_frame) 없음 — Widget 분할 불가", flush=True)
        except Exception:
            pass
        return False
    ...
    try:
        ...
        try:
            print(f"[LAM multi-sim] Viewport get_frame Widget 분할 실패: {exc}", flush=True)
        except Exception:
            pass
        teardown_split_widget_host(ext)
        return False
    ...
    try:
        ext._lam_split_widget_tiles = tiles
        ext._lam_split_viewport_host_window = vw_host
        ext._lam_split_widget_host_win = vw_host
        ext._lam_split_used_widget_layout = True
        ext._lam_split_used_dock_layout = False
        ext._lam_active_widget_tile = "Viewport"
        ext._lam_split_main_viewport_window = vw_host
        ext._lam_main_stage_connected = False
        ext._lam_aux_stage_connected = False
        ext._lam_aux_vw_materialized = False
        ext._lam_widget_stage_sync_key = None
        ext._lam_widget_split_ready = False
    ...
    try:
        print(
            "[LAM multi-sim] ViewportWidget shell 적용 — 화면1=#1, 화면2=master_2 후 deferred #2",
            flush=True,
        )
    except Exception:
        pass
