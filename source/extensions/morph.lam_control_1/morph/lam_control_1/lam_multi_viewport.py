...
"""
lam_multi_viewport.py — 멀티 시뮼 화면 분할
...
- **1화면**: `teardown_lam_multi_viewports` 로 보조 뷰·컨텍스트 정리 후 Viewport 복원.
...
- **본 모듈(lam_multi_viewport)**: Kit 뷰포트/Workspace 창·Dock·보조 ``omni.usd`` 컨텍스트·스테이지 복제/래퍼·
...
- ``_restore_main_viewport_layout`` / ``teardown_lam_multi_viewports`` : 1화면 복원·전체 철거.
...
- ``_viewport_window_name_for_screen`` / ``_resolve_viewport_window_for_workspace_name`` : 뷰포트 윈도우 조회.
...
def _use_split_composed_export(ext: Any = None) -> bool:
    ...
    """
    try:
        from .lam_split_composed_loader import split_dual_usd_paths_enabled
    ...
    if ext is not None:
        try:
            from .lam_split_composed_loader import composed_split_snapshot_ready
...
def _split_layout_usd_key(ext: Any) -> str:
    ...
    try:
        from .lam_split_composed_loader import _main_stage_cache_key
        ...
        pass
    try:
        return str(getattr(ext, "_lam_last_loaded_usd_path", "") or "").strip()
    except Exception:
        return ""
...
def invalidate_split_layout_cache(ext: Any) -> None:
    ...
    try:
        ext._lam_split_layout_usd_key = None
    except Exception:
        pass
    try:
        from .lam_split_composed_loader import clear_split_composed_export_cache
...
def sim_viewport_split_dock_enabled() -> bool:
    ...
    """
    try:
        from .lam_multi_viewport_widget import sim_viewport_split_widget_enabled
...
def channel_count_for_split(split_n: int) -> int:
    ...
    try:
        from .lam_sim_control_defaults import MAX_VIEWPORT_SPLIT_COUNT
...
def split_layout_description(split_n: int) -> str:
    ...
    try:
        ...
        pass
    if n == 2:
        return "2분할: Viewport Dock — LAM_SimSplit_1(우 50%)"
    if n == 3:
...
def _split_window_name(aux_index_1based: int) -> str:
    ...
    return f"LAM_SimSplit_{int(aux_index_1based)}"
...
def viewport_split_user_resize_locked() -> bool:
    ...
    try:
        from .lam_sim_control_defaults import LOCK_VIEWPORT_SPLIT_USER_RESIZE
...
def apply_viewport_split_tab_chrome(n: int, ext: Any = None) -> None:
    ...
    try:
        from .lam_multi_viewport_widget import hide_viewport_workspace_tab_chrome, is_split_widget_layout_active
        from .lam_extension_singleton import get_lam_extension_instance

        e = ext if ext is not None else get_lam_extension_instance()
        ...
        pass
    try:
        from .lam_kit_chrome_visibility import apply_viewport_dock_tab_bars_hidden
...
_SPLITTER_GUARD_WIN_TITLE = "LAM_SimSplit_SplitterGuard"
...
def _apply_splitter_guard_geometry(ext: Any, rect: Tuple[float, float, float, float]) -> None:
    gx, gy, gw, gh = rect
    win = getattr(ext, "_lam_split_splitter_guard_win", None)
    if win is None:
        return
...
def _ensure_splitter_guard_window(ext: Any) -> Any:
    ...
    try:
        ...
        return None
    try:
        ext._lam_split_splitter_guard_win = win
    except Exception:
        pass
    try:
        with win.frame:
...
def _tick_viewport_split_splitter_guard(ext: Any) -> None:
    ...
    try:
        ...
        n = 1
    if n < 2 or not bool(getattr(ext, "_lam_split_used_dock_layout", False)):
        _hide_splitter_guard_window(ext)
        return
    ...
    last = getattr(ext, "_lam_split_splitter_guard_last_rect", None)
    need_apply = True
    ...
    if need_apply:
        ...
        try:
            ext._lam_split_splitter_guard_last_rect = tuple(float(x) for x in rect)
        except Exception:
            pass
...
def _install_viewport_split_splitter_guard(ext: Any) -> None:
    ...
    try:
        sub_ref[0] = kit_app.get_app().get_post_update_event_stream().create_subscription_to_pop(
            _on_tick,
            name="morph.lam_control_1.split_splitter_guard",
        )
        ext._lam_split_splitter_guard_sub = sub_ref[0]
    except Exception:
        pass
...
def teardown_viewport_split_resize_lock(ext: Any) -> None:
    """분할선 가드 구독·창 정리."""
    sub = getattr(ext, "_lam_split_splitter_guard_sub", None)
    try:
        ext._lam_split_splitter_guard_sub = None
    except Exception:
        pass
    if sub is not None:
        try:
...
async def _enforce_equal_split_grid_async(
    ...
    for _ in range(3):
        if int(getattr(ext, "_lam_multi_viewport_apply_token", 0) or 0) != token:
            return (vx, vy, vw, vh)
    ...
    try:
        ext._lam_split_saved_viewport_rect = (int(vx), int(vy), int(vw), int(vh))
    except Exception:
        pass
    try:
        print(
            f"[LAM multi-sim] 동일 크기 격자 배치 OK area=({vx},{vy},{vw}x{vh}) tiles={n}",
            flush=True,
        )
    except Exception:
        pass
...
def _split_viewport_api(win_name: str) -> Any:
    ...
    try:
        from .lam_multi_viewport_widget import get_split_viewport_api, is_split_widget_layout_active
        from .lam_extension_singleton import get_lam_extension_instance
...
def _sync_entries_from_widget_tiles(ext: Any) -> None:
    ...
    if not isinstance(tiles, dict):
        return
    entries = list(getattr(ext, "_lam_multi_viewport_entries", []) or [])
    for ent in entries:
    ...
    try:
        ext._lam_multi_viewport_entries = entries
    except Exception:
        pass
...
def _sync_viewport_resolution_from_workspace_window(win_name: str) -> None:
    ...
    try:
        ...
        ext = get_lam_extension_instance()
        if ext is not None and is_split_widget_layout_active(ext):
            from .lam_multi_viewport_widget import sync_split_widget_fill_frame
...
def _split_tile_index_from_win_name(win_name: str) -> int:
    """``Viewport`` → 0, ``LAM_SimSplit_1`` → 1 …"""
    wn = str(win_name or "").strip()
    if wn == "Viewport":
        return 0
    if wn.startswith("LAM_SimSplit_"):
        try:
...
def _should_skip_kit_panel_auto_show(win_name: str, ext: Any = None) -> bool:
    ...
    if ext is None:
        return False
    try:
        from .lam_kit_chrome_visibility import is_kit_chrome_hidden
...
def reapply_split_layout_sync(ext: Any, n: int) -> None:
    ...
    if sn <= 1:
        return
    try:
        from .lam_multi_viewport_widget import is_split_widget_layout_active, sync_split_widget_fill_frame

        if is_split_widget_layout_active(ext):
            sync_split_widget_fill_frame(ext, sn)
            try:
                from .lam_multi_viewport_widget import ensure_viewport_workspace_tab_visible
        ...
        pass
    if bool(getattr(ext, "_lam_split_used_dock_layout", False)):
...
def set_viewport_fill_frame_for_split_count(split_n: int, fill: bool) -> None:
    ...
    """
    try:
        from .lam_hyview_stream import is_lam_hyview_stream_layout_locked, bridge_stream_skip

        if is_lam_hyview_stream_layout_locked():
            bridge_stream_skip(
                "fill_frame",
                "layout_locked",
                split_n=int(split_n),
...
async def _apply_split_dock_layout(
    ...
    if not await _wait_workspace_windows_ready(ext, token, aux_names):
        try:
            print("[LAM multi-sim] Dock 분할: 보조 창 Workspace 등록 대기 시간 초과", flush=True)
        except Exception:
            pass
        return False
    for child, target, pos, ratio in ops:
        ...
        if child_w is None:
            try:
                print(f"[LAM multi-sim] Dock 분할: 보조 창 없음 name={child!r}", flush=True)
            except Exception:
                pass
            return False
        if target_w is None:
            try:
                print(f"[LAM multi-sim] Dock 분할: 기준 창 없음 name={target!r}", flush=True)
            except Exception:
                pass
            return False
        ...
        if not ok:
            try:
                print(
                    f"[LAM multi-sim] Dock 분할 실패 child={child!r} target={target!r} ratio={ratio} "
    ...
    if not await _wait_aux_windows_docked(ext, token, aux_names, max_frames=72):
        if not _aux_windows_dock_plausible(n):
            if warn_on_dock_miss:
                try:
                    print(
                        "[LAM multi-sim] Dock: docked 플래그 미확인 — 격자 폴백 또는 재시도",
                        flush=True,
                    )
                except Exception:
                    pass
    ...
    for nm in aux_names:
        hidden = False
        for ent in list(getattr(ext, "_lam_multi_viewport_entries", []) or []):
        ...
        _sync_viewport_resolution_from_workspace_window(str(nm))
    try:
        print("[LAM multi-sim] Viewport Dock 분할 적용 완료 (dock_in)", flush=True)
    except Exception:
        pass
    try:
        from .lam_kit_chrome_visibility import apply_viewport_dock_tab_bars_hidden
...
def _main_usd_path_for_clone(ext: Any) -> Optional[str]:
    ...
    try:
        from .lam_split_composed_loader import resolve_split_master_usd_path

        return resolve_split_master_usd_path(ext)
    except Exception:
        pass
    try:
        lp = str(getattr(ext, "_lam_last_loaded_usd_path", "") or "").strip()
        if lp:
            return lp
    except Exception:
        pass
...
def startup_layout_first_active(ext: Any) -> bool:
    ...
    return bool(getattr(ext, "_lam_startup_layout_first_active", False))
...
def _startup_split_relayout_suppressed(ext: Any) -> bool:
    ...
    if startup_dual_orchestration_active(ext):
        return True
    if bool(getattr(ext, "_lam_startup_aux_load_inflight", False)):
        return True
    return False
...
def startup_dual_layout_settled(ext: Any) -> bool:
    ...
    return bool(getattr(ext, "_lam_startup_dual_layout_settled", False))
...
def preserve_split_layout_during_startup(ext: Any) -> bool:
    ...
    if bool(getattr(ext, "_lam_startup_aux_load_inflight", False)):
        return True
    if bool(getattr(ext, "_lam_defer_master_autoload_until_dual_layout", False)):
        return True
    return False
...
def startup_dual_orchestration_active(ext: Any) -> bool:
    ...
    return bool(getattr(ext, "_lam_startup_dual_orchestration_active", False))


def _begin_startup_dual_orchestration(ext: Any) -> None:
    try:
        ext._lam_startup_dual_orchestration_active = True
        ext._lam_startup_pending_chrome_relayout = None
    except Exception:
        pass
...
async def _ensure_split_layout_geometry_ready(ext: Any, token: int, n: int) -> None:
    ...
    if not sim_viewport_split_dock_enabled():
        ...
        try:
            ext._lam_split_used_dock_layout = False
        except Exception:
            pass
        return
    for _ in range(4):
    ...
    try:
        ext._lam_split_used_dock_layout = bool(docked_ok)
    except Exception:
        pass
    if _aux_windows_actually_docked(n):
        return
    try:
        print("[LAM multi-sim] layout-first: Dock 미확인 → 동일 크기 격자 폴백", flush=True)
    except Exception:
        pass
    for ti in range(1, n):
        _undock_workspace_window(_split_window_name(ti))
...
async def _finish_startup_dual_orchestration(ext: Any) -> None:
    ...
    try:
        from .lam_kit_chrome_visibility import (
        ...
        pass
    try:
        ext._lam_startup_dual_orchestration_active = False
        ext._lam_startup_pending_chrome_relayout = None
        ext._lam_startup_dual_layout_settled = False
    except Exception:
        pass
    for _ in range(4):
        await kit_app.get_app().next_update_async()
    try:
        ...
        sn = 2
    if sn > 1:
        tok = int(getattr(ext, "_lam_multi_viewport_apply_token", 0) or 0)
        try:
            from .lam_multi_viewport_widget import is_split_widget_layout_active, sync_split_widget_fill_frame
            ...
            if is_split_widget_layout_active(ext):
                sync_split_widget_fill_frame(ext, sn)
                try:
                    from .lam_multi_viewport_widget import finalize_widget_split_startup
...
def _startup_split_usd_path(ext: Any) -> Optional[str]:
    ...
    if p:
        return p
    try:
        from .lam_window import default_load_usd_path
...
def schedule_deferred_aux_usd_load_after_master(ext: Any) -> None:
    ...
    if not bool(getattr(ext, "_lam_split_deferred_aux_load_pending", False)):
        return
    try:
        ext._lam_split_deferred_aux_load_pending = False
    except Exception:
        pass

    async def _go() -> None:
        try:
            ext._lam_startup_aux_load_inflight = True
        except Exception:
            pass
        try:
            ...
            tok = int(getattr(ext, "_lam_multi_viewport_apply_token", 0) or 0)
            try:
                print("[LAM multi-sim] 레이아웃 완료 → 화면2 USD 로드 시작", flush=True)
            except Exception:
                pass
            ...
            try:
                ext._lam_startup_aux_load_inflight = False
            except Exception:
                pass
            try:
...
def _register_session_layer_path(ext: Any, path: str) -> None:
    ...
    if not path:
        return
    try:
        lst = list(getattr(ext, "_lam_split_session_layer_paths", []) or [])
        if path not in lst:
            lst.append(path)
        ext._lam_split_session_layer_paths = lst
    except Exception:
        pass


def _unlink_split_session_files(ext: Any) -> None:
    for p in list(getattr(ext, "_lam_split_session_layer_paths", []) or []):
        try:
            ...
            pass
    try:
        ext._lam_split_session_layer_paths = []
    except Exception:
        pass
...
async def _clone_usd_for_aux_tile(usd_path: str, ext: Any, token: int, ti: int) -> Tuple[Optional[str], str]:
    ...
    dest = os.path.normpath(
        os.path.join(
            tempfile.gettempdir(),
            f"morph_lam_clone_aux_{ti}_{token}_{os.getpid()}{suf}",
        )
    )
    dest_uri = Path(dest).as_uri()
    src_uri = usd_path
    try:
        from .lam_split_composed_loader import usd_path_for_omni_client
    ...
    try:
        print(f"[LAM multi-sim] 보조 타일 {ti}: 분할용 USD 복제 완료 ({os.path.getsize(dest)} bytes)", flush=True)
    except Exception:
        pass
    return dest, ""
...
def _make_aux_wrapper_root_layer(usd_path: str, ext: Any, token: int, ti: int) -> Tuple[Optional[str], str]:
    ...
    wrap_path = os.path.normpath(
        os.path.join(
            tempfile.gettempdir(),
            f"morph_lam_wrap_aux_{ti}_{token}_{os.getpid()}.usda",
        )
    )
    try:
        from pxr import Sdf

        lyr = Sdf.Layer.CreateAnonymous("morph_lam_wrap")
...
def _export_empty_session_usda(sess_path: str) -> Tuple[bool, str]:
    try:
        ...
        anon = Sdf.Layer.CreateAnonymous("morph_lam_aux_sess")
...
def _make_aux_shell_stage_path(ext: Any, token: int, ti: int) -> Tuple[Optional[str], str]:
    ...
    path = os.path.normpath(
        os.path.join(
            tempfile.gettempdir(),
            f"morph_lam_shell_aux_{ti}_{token}_{os.getpid()}.usda",
        )
    )
    try:
        from pxr import Usd
...
async def _ctx_open_stage_path(ctx: Any, root: str, sess_path: Optional[str]) -> Tuple[bool, str]:
    ...
    from .lam_viewport_open_draw import resume_viewport_draw, suspend_viewport_draw
...
async def _open_aux_stage_with_unique_session(
    ...
    *,
    ...
    1) **타일 전용 복제/스냅샷**(``morph_lam_clone_aux_*`` / ``morph_lam_composed_aux_*``)은 바로 연다.
    ...
    if root_path is None and _is_preopened_aux_stage_path(p_in, ext, ti):
        ...
        if clone_path:
            ...
            try:
                print(
                    f"[LAM multi-sim] 보조 USD 복제 실패 tile={ti} (래퍼 폴백 금지 — subLayer 공유 crash): {cerr}",
                    flush=True,
                )
            except Exception:
                pass
    ...
    if _use_multi_split_session_layer() and not composed_open:
        sess_path = os.path.normpath(
            os.path.join(
                tempfile.gettempdir(),
                f"morph_lam_sess_aux_{ti}_{token}_{os.getpid()}.usda",
            )
        )
        ...
        if ok:
            return True, ""
        try:
            print(f"[LAM multi-sim] 보조: 루트+session 실패 → 루트 단독 재시도 ({err})", flush=True)
        except Exception:
            pass
...
def _log_split_stage_not_shared_with_main(ctx_names: List[str]) -> None:
    ...
    try:
        ...
        for nm in ctx_names:
            ...
            if st is main:
                print(
                    f"[LAM multi-sim] 경고: 보조 컨텍스트 {nm!r} 가 메인과 동일 Usd.Stage 객체를 공유합니다.",
                    flush=True,
                )
    except Exception:
        pass
...
def _is_lam_composed_snapshot_path(path: str) -> bool:
    """``export_main_composed_stage_to_temp`` / ``get_or_export_main_composed_stage`` 결과 경로."""
    try:
        base = os.path.basename(str(path or ""))
    except Exception:
        return False
    return base.startswith("morph_lam_composed_aux_")


def _is_lam_clone_aux_path(path: str) -> bool:
    """``_clone_usd_for_aux_tile`` 로 만든 타일 전용 복제 파일."""
    try:
        base = os.path.basename(str(path or ""))
    except Exception:
        return False
    return base.startswith("morph_lam_clone_aux_")


def _is_preopened_aux_stage_path(path: str, ext: Any = None, ti: int = 0) -> bool:
    """이미 타일별로 복제·Export 된 경로 — 재복제·래퍼(subLayer 공유) 없이 바로 연다."""
    if _is_lam_composed_snapshot_path(path) or _is_lam_clone_aux_path(path):
        return True
    if ext is not None:
        try:
            from .lam_split_composed_loader import aux_usd_path_is_direct_open
...
def _is_split_aux_stage_file(path: str, ext: Any = None, ti: int = 0) -> bool:
    ...
    try:
        ...
        return False
    return base.startswith("morph_lam_composed_aux_") or base.startswith("morph_lam_clone_aux_")
...
def _log_viewport_usd_context_bind(win_name: str, expect_ctx: str) -> None:
    ...
    try:
        ...
        if got and expect_ctx and str(got) != str(expect_ctx):
            print(
                f"[LAM multi-sim] 뷰포트 컨텍스트 불일치: 창={win_name!r} api={got!r} 기대={expect_ctx!r}",
                flush=True,
            )
    except Exception:
        pass
...
def _schedule_split_aux_navigation_hold(ext: Any, n: int, token: int, *, ticks: int = 36) -> None:
    ...
    try:
        sub_ref[0] = kit_app.get_app().get_post_update_event_stream().create_subscription_to_pop(
            _tick,
            name="morph.lam_control_1.split_aux_nav_hold",
        )
        try:
...
def _workspace_show_named_window(name: str, visible: bool) -> None:
    ...
    if bool(visible) and wn.startswith("LAM_SimSplit"):
        try:
            from .lam_multi_viewport_widget import sim_viewport_split_widget_enabled
...
def _restore_main_viewport_layout(ext: Any) -> None:
    ...
    ``_lam_split_saved_viewport_rect``(분할 중 캡처)는 Dock 분할 후 값이라
    1화면 복귀 시 Console/Content 를 덮을 수 있어 **사용하지 않는다**.
    """
    panels: Dict[str, Tuple[int, int, int, int]] = dict(
        getattr(ext, "_lam_split_saved_panel_layout", None) or {}
    )
...
def _bump_split_teardown_restore_generation(ext: Any) -> int:
    ...
    try:
        ext._lam_split_restore_gen = int(_split_viewport_restore_gen)
    except Exception:
        pass
...
def _restore_viewport_after_split_teardown(ext: Any, *, schedule_deferred_restore: bool = True) -> None:
    ...
    used_dock = bool(getattr(ext, "_lam_split_used_dock_layout", False))
    if used_dock:
...
def teardown_lam_multi_viewports(ext: Any, *, skip_deferred_restore: bool = False) -> None:
    ...
    _cancel_split_aux_navigation_hold(ext)
    try:
        from .lam_multi_viewport_widget import is_split_widget_layout_active, teardown_split_widget_host
    ...
    try:
        from .lam_split_composed_loader import release_aux_split_runtimes
    ...
    try:
        ...
        pass
    entries: List[Dict[str, Any]] = list(getattr(ext, "_lam_multi_viewport_entries", []) or [])
    ...
    try:
        ext._lam_multi_viewport_entries = []
    except Exception:
        pass
    ...
    try:
        ext._lam_split_saved_viewport_rect = None
    except Exception:
        pass
    ...
    try:
        ext._lam_split_main_viewport_window = None
    except Exception:
        pass
...
def split_layout_needs_reapply(ext: Any) -> bool:
    ...
    if n <= 1:
        return False
    try:
        from .lam_multi_viewport_widget import is_split_widget_layout_active, split_widget_layout_healthy
    ...
    if not _split_aux_layout_healthy(ext, n):
        return True
    if bool(getattr(ext, "_lam_split_used_widget_layout", False)):
        return False
    if not bool(getattr(ext, "_lam_split_used_dock_layout", False)):
        return True
    return False
...
def _capture_kit_panel_layout(ext: Any) -> None:
    ...
    for nm in ("Viewport", "Console", "Content", "Stage", "Property"):
        ...
        if r is not None and int(r[2]) >= 8 and int(r[3]) >= 8:
            panels[str(nm)] = r
    try:
        ext._lam_split_saved_panel_layout = panels
    except Exception:
        pass
...
def _capture_split_tile_bbox(ext: Any) -> Tuple[int, int, int, int]:
    ...
    Viewport·LAM_SimSplit_* 만 이후 단계에서 리사이즈한다.
    """
    panels: Dict[str, Tuple[int, int, int, int]] = dict(
        getattr(ext, "_lam_split_saved_panel_layout", None) or {}
    )
...
def _get_split_tile_bbox(ext: Any) -> Tuple[int, int, int, int]:
    ...
    r = getattr(ext, "_lam_split_saved_viewport_rect", None)
...
def _read_split_cluster_union_rect(n: int) -> Tuple[int, int, int, int]:
    """
    분할 타일(``Viewport`` + ``LAM_SimSplit_*``) 화면 좌표의 합집합.
        ...
        if bool(getattr(ext, "_lam_split_used_dock_layout", False)):
            ...
            asyncio.ensure_future(
                _enforce_equal_split_grid_async(
                    ext,
                    int(getattr(ext, "_lam_multi_viewport_apply_token", 0) or 0),
                    n,
...
def schedule_split_layout_refresh_for_chrome_change(ext: Any, menus_hidden: bool) -> None:
    ...
    if startup_dual_orchestration_active(ext):
        try:
            ext._lam_startup_pending_chrome_relayout = bool(menus_hidden)
        except Exception:
            pass
        return
    ...
    async def _go() -> None:
        ...
        try:
            ...
            pass
        try:
            from .lam_kit_chrome_visibility import apply_viewport_dock_tab_bars_hidden, is_streaming_deployment
...
async def _assign_split_cameras_after_layout(
    ...
    try:
        from .lam_multi_viewport_widget import assign_widget_split_cameras, is_split_widget_layout_active
...
def _split_layout_already_active(ext: Any, n: int) -> bool:
    ...
    try:
        ...
        if cur != sn or sn <= 1:
            return False
        built_key = str(getattr(ext, "_lam_split_layout_usd_key", "") or "").strip()
...
async def _hydrate_aux_split_tile_background(
    ...
    *,
    ...
    if int(getattr(ext, "_lam_multi_viewport_apply_token", 0) or 0) != token:
        return
    try:
        from .lam_split_composed_loader import hydrate_split_screen_composed_stage_async
        ...
        try:
            print(
                f"[LAM multi-sim] 보조 합성 USD hydrate(백그라운드) 실패 screen={screen_1based}: {exc}",
                flush=True,
            )
        except Exception:
            pass
...
async def _finalize_aux_split_tile_after_open(
    ...
    *,
    ...
    if int(getattr(ext, "_lam_multi_viewport_apply_token", 0) or 0) != token:
        return
    # LAM: EP 포트 visibility (TBS tbs_ep_port_visibility) 미포팅 — 스킵
    try:
    ...
    try:
        ...
        try:
            from .lam_multi_viewport_widget import is_split_widget_layout_active, request_aux_widget_materialize
...
def _destroy_aux_split_tile(ext: Any, ti: int, entries: List[Dict[str, Any]], ctx_names: List[str]) -> None:
    ...
    ctx_name = f"morph_lam_split_aux_{ti}"
    ...
    try:
        from .lam_split_composed_loader import release_split_runtime_for_screen
...
def _collect_pending_aux_tile_hydrates(
    ...
    try:
        ...
        return [], []
    if sn <= 1:
        return list(getattr(ext, "_lam_multi_viewport_entries", []) or []), []
    ...
    for ent in entries:
        ...
        if ci < 1:
            continue
        ctx_name = str(ent.get("context_name") or f"morph_lam_split_aux_{ci}").strip()
        if ctx_name:
...
async def _apply_aux_bg_load_layout_finish(
    ...
    *,
    ...
    if startup_dual_layout_settled(ext) and skip_dock_reapply:
        ...
        return
    try:
        from .lam_multi_viewport_widget import is_split_widget_layout_active, _destroy_all_aux_workspace_windows
...
async def wake_main_viewport_after_master_open(ext: Any, n: int = 0) -> None:
    ...
    try:
        ...
        sn = 2
    try:
        from .lam_multi_viewport_widget import connect_widget_tile_main_stage, is_split_widget_layout_active
...
async def _clone_aux_paths_parallel(
    ...
    *,
    ...
    for ti, path, composed, err in results:
        if not path:
            try:
                print(f"[LAM multi-sim] 보조 타일 {ti} USD 병렬 복제 실패: {err}", flush=True)
            except Exception:
                pass
            return None
...
async def _refresh_aux_split_stages_async(ext: Any, n: int, token: int, usd_path: str) -> None:
    ...
    if len(ctx_names) < sn - 1 or len(entries) < sn:
        try:
            print("[LAM multi-sim] 분할 빠른 갱신 불가 → 전체 재빌드", flush=True)
        except Exception:
            pass
        teardown_lam_multi_viewports(ext)
        await _post_teardown_rebuild_split(ext, sn, token, usd_path, prev_n=1)
        return

    try:
        print(f"[LAM multi-sim] 분할 빠른 갱신: reopen+hydrate (n={sn})", flush=True)
    except Exception:
        pass

    try:
        from .lam_split_composed_loader import resolve_split_aux_usd_path, split_dual_usd_paths_enabled
    ...
    if use_composed:
        try:
            from .lam_split_composed_loader import get_or_export_main_composed_stage
    ...
    def _nav_after_hydrate() -> None:
        ...
        try:
            win = getattr(ext, "_lam_window", None) or getattr(ext, "_window", None)
            if win is not None and hasattr(win, "refresh_csv_sim_play_runtimes"):
    ...
    try:
        ext._lam_split_layout_usd_key = _split_layout_usd_key(ext)
    except Exception:
        pass
...
async def _resolve_aux_tile_usd_path(
    ...
    *,
    ...
    """
    try:
        from .lam_split_composed_loader import resolve_split_aux_usd_path

        aux_pre = resolve_split_aux_usd_path(ext, ti)
        if aux_pre:
            try:
                print(
                    f"[LAM multi-sim] 보조 타일 {ti}: dual-path USD (복제 생략) path={aux_pre}",
                    flush=True,
                )
            except Exception:
                pass
...
async def _materialize_dock_windows_for_widget_aux_entries(
    ...
    for ent in pending:
        ...
        try:
            ...
            ti = 0
        ctx_name = str(ent.get("context_name") or f"morph_lam_split_aux_{ti}")
...
async def _provision_aux_split_tile(
    ...
    *,
    ...
    if ctx is None:
        try:
            print(f"[LAM multi-sim] USD 컨텍스트 생성 실패: {ctx_name}", flush=True)
        except Exception:
            pass
        return False
    ...
    if defer_stage_load:
        if _dual_path_split_defer_skip_shell(ext):
            ...
            try:
                print(
                    f"[LAM multi-sim] 보조 타일 {ti}: dual-path defer (shell 생략, 1회 open)",
                    flush=True,
                )
            except Exception:
                pass
            ...
            if not shell_path:
                try:
                    print(f"[LAM multi-sim] 보조 타일 {ti} shell stage 실패: {shell_err}", flush=True)
                except Exception:
                    pass
                _release_usd_context_names([ctx_name])
                if ctx_name in ctx_names:
        ...
        if not aux_usd:
            try:
                print(f"[LAM multi-sim] 보조 타일 {ti} USD 준비 실패: {usd_err}", flush=True)
            except Exception:
                pass
            _release_usd_context_names([ctx_name])
            if ctx_name in ctx_names:
    ...
    if not ok_open:
        try:
            print(f"[LAM multi-sim] 보조 스테이지 열기 실패 ctx={ctx_name} err={err_open}", flush=True)
        except Exception:
            pass
        _release_usd_context_names([ctx_name])
        if ctx_name in ctx_names:
    ...
    if create_viewport_window is not None:
        try:
            ...
            try:
                print(f"[LAM multi-sim] create_viewport_window 실패 name={wname} err={e}", flush=True)
            except Exception:
                pass
    ...
    if vp_obj is None:
        try:
            ...
            try:
                print(f"[LAM multi-sim] ViewportWindow 폴백도 실패 name={wname} err={e2}", flush=True)
            except Exception:
                pass
            _release_usd_context_names([ctx_name])
            if ctx_name in ctx_names:
    ...
    # play/overlay 가 get_viewport_from_window_name 실패해도 찾을 수 있게 캐시
    try:
        cache = getattr(ext, "_lam_split_aux_viewport_by_name", None)
        if not isinstance(cache, dict):
            cache = {}
            ext._lam_split_aux_viewport_by_name = cache
        cached_obj = None
        for ent in reversed(entries):
            if str(ent.get("win_name") or "") == str(wname):
                cached_obj = ent.get("viewport_window") or ent.get("kit_vp") or ent.get("window")
                break
        if cached_obj is not None:
            cache[str(wname)] = cached_obj
            print(
                f"[LAM multi-sim] aux viewport cache name={wname} "
                f"type={type(cached_obj).__name__} "
                f"get_frame={callable(getattr(cached_obj, 'get_frame', None))} "
                f"has_api={getattr(cached_obj, 'viewport_api', None) is not None}",
                flush=True,
            )
    except Exception:
        pass
...
async def _load_aux_split_stages_background_impl(
    ...
    *,
    ...
    try:
        ...
        print(
            f"[LAM multi-sim] bg load: mode={mode} "
    ...
    if clone_map is None:
        try:
            print("[LAM multi-sim] bg load: USD 복제 실패", flush=True)
        except Exception:
            pass
        return
    ...
    for ti in range(1, n):
        ...
        if not ok_open:
            try:
                print(
                    f"[LAM multi-sim] bg load: tile={ti} open FAIL err={err_open}",
                    flush=True,
                )
            except Exception:
                pass
    ...
    on_done = getattr(ext, "_lam_split_nav_after_hydrate_fn", None)
    ...
    if serialize_gpu:
        ...
        try:
            ext._lam_split_layout_usd_key = _split_layout_usd_key(ext)
            ext._lam_layout_first_aux_load_done = True
        except Exception:
            pass
        return
...
async def _complete_startup_layout_first_async(ext: Any, n: int, token: int) -> None:
    ...
    try:
        print(
            "[LAM multi-sim] layout-first: 2분할 Dock·격자 배치 완료 — USD 로드 대기",
            flush=True,
        )
    except Exception:
        pass

    fn = getattr(ext, "_lam_on_dual_layout_ready_fn", None)
    if callable(fn):
        try:
            ext._lam_on_dual_layout_ready_fn = None
        except Exception:
            pass
        try:
            fn(ext)
            ...
            try:
                print(f"[LAM multi-sim] layout-first ready callback failed: {exc}", flush=True)
            except Exception:
                pass

    try:
        ext._lam_startup_layout_first_active = False
    except Exception:
        pass
...
async def _finish_split_layout_after_tiles(
    ...
    try:
        ...
        if widget_only:
            ...
            if widget_ok:
                ...
                try:
                    print(
                        "[LAM multi-sim] ViewportWidget 분할 실패 — "
    ...
    if not widget_ok and not widget_only:
        ...
        try:
            ext._lam_split_used_dock_layout = bool(docked_ok)
            if docked_ok:
                ext._lam_split_used_widget_layout = False
        except Exception:
            pass
    ...
    if widget_ok:
        for _ in range(2):
            ...
            await kit_app.get_app().next_update_async()
        try:
            print("[LAM multi-sim] 분할 레이아웃: ViewportWidget shell (Dock 미사용)", flush=True)
        except Exception:
            pass
    elif docked_ok:
        for _ in range(8):
        ...
        for ti in range(n):
            ...
            _sync_viewport_resolution_from_workspace_window(nm)
        try:
            print("[LAM multi-sim] 분할 레이아웃: Viewport Dock (Console/Content 유지)", flush=True)
        except Exception:
            pass
    else:
        await _enforce_equal_split_grid_async(
    ...
    try:
        ...
        pass
    if n >= 2 and not bool(getattr(ext, "_lam_split_used_widget_layout", False)):
        apply_viewport_split_tab_chrome(n)
        if viewport_split_user_resize_locked():
            apply_viewport_split_user_resize_lock(ext)
    ...
    if layout_first_shell:
        ...
        try:
            ext._lam_startup_dual_layout_settled = True
        except Exception:
            pass
        for _ in range(6):
            if int(getattr(ext, "_lam_multi_viewport_apply_token", 0) or 0) != token:
        ...
        if not widget_active:
            ...
            try:
                from .lam_multi_viewport_widget import _destroy_all_aux_workspace_windows
    ...
    def _nav_after_all_hydrate() -> None:
        ...
        try:
            ...
            pass
        _apply_split_navigation_to_aux(ext, n, token, hold_ticks=96)
        # Dock 모드: hydrate 후 화면2+ FOUP/기기 라벨을 보조 창에 재동기화
        try:
            win = getattr(ext, "_lam_window", None) or getattr(ext, "_window", None)
            if win is not None and hasattr(win, "refresh_csv_sim_play_runtimes"):
                win.refresh_csv_sim_play_runtimes()
    ...
    if skip_hydrate:
        if not layout_first_shell:
            try:
                print(
                    "[LAM multi-sim] 분할 레이아웃 완료 — 보조 스테이지 로드는 백그라운드",
                    flush=True,
                )
            except Exception:
                pass
    ...
    try:
        ext._lam_split_nav_after_hydrate_fn = _nav_after_all_hydrate
    except Exception:
        pass
    ...
    try:
        print(
            f"[LAM multi-sim] 분할={n} | 첫 타일=메인 Viewport | 보조 컨텍스트 {len(ctx_names)}개 | "
        ...
        pass
    try:
        ext._lam_split_main_viewport_window = _resolve_viewport_window_for_workspace_name("Viewport")
    except Exception:
        try:
            ext._lam_split_main_viewport_window = None
        except Exception:
            pass
    try:
        from .lam_split_composed_loader import register_main_composed_runtime
...
async def _grow_split_async(ext: Any, n: int, prev_n: int, token: int, usd_path: str) -> None:
    ...
    for ti in range(prev_n, n):
        ...
        if not ok:
            try:
                print(f"[LAM multi-sim] 분할 확대 실패 tile={ti} → 전체 재빌드", flush=True)
            except Exception:
                pass
            teardown_lam_multi_viewports(ext)
...
async def _build_multi_split_async(ext: Any, n: int, token: int, usd_path: str, *, prev_n: int = 1) -> None:
    ...
    if main_vp is None:
        try:
            print("[LAM multi-sim] Workspace 에 'Viewport' 창이 없어 분할을 적용하지 않습니다.", flush=True)
        except Exception:
            pass
        return
    ...
    if startup_layout_first_active(ext) or startup_dual_orchestration_active(ext):
        try:
            print(f"[LAM multi-sim] 분할 빌드: layout-first shell (n={n})", flush=True)
        except Exception:
            pass
        for ti in range(1, n):
            if int(getattr(ext, "_lam_multi_viewport_apply_token", 0) or 0) != token:
        ...
        try:
            ...
            ext._sim_multi_context_names = ctx_names
            ext._sim_viewport_split_count = n
            ext._lam_split_deferred_aux_load_pending = True
        except Exception:
            pass
    ...
    try:
        ...
        print(f"[LAM multi-sim] 분할 빌드: {mode} (n={n})", flush=True)
    except Exception:
        pass
...
def _apply_sim_viewport_split_layout_impl(ext: Any, n: int) -> None:
    ...
    if n <= 1:
        ...
        try:
            ...
            pass
        try:
            ext._lam_split_3d_disabled_notice = False
        except Exception:
            pass
        try:
            print(f"[LAM multi-sim] 분할=1 | {split_layout_description(1)}", flush=True)
        except Exception:
            pass
        notify_sim_split_ui_sync(ext)
        return
    ...
    if not sim_viewport_split_3d_enabled():
        ...
        try:
            ...
            pass
        try:
            if not getattr(ext, "_lam_split_3d_disabled_notice", False):
                ext._lam_split_3d_disabled_notice = True
                print(
                    "[LAM multi-sim] 보조 3D 뷰가 꺼져 있습니다(TBS_SIM_VIEWPORT_SPLIT_3D=0 등). "
            ...
            pass
        try:
            print(f"[LAM multi-sim] 분할={n} | {split_layout_description(n)} (3D 타일 미생성)", flush=True)
        except Exception:
            pass
        return

    if not getattr(ext, "_lam_multi_split_usd_ready", False):
        if not preserve_split_layout_during_startup(ext):
            ...
            try:
                ...
                pass
            try:
                print("[LAM multi-sim] LAM Multi-USD Load Load 로 연 스테이지가 있을 때만 분할 뷰를 씁니다.", flush=True)
            except Exception:
                pass
            return
    ...
    if not usd_path:
        ...
        try:
            ...
            pass
        try:
            print("[LAM multi-sim] 복제할 USD 경로를 찾지 못해 분할 뷰를 만들지 않습니다.", flush=True)
        except Exception:
            pass
        return
    ...
    if (
        ...
        and str(getattr(ext, "_lam_split_layout_usd_key", "") or "").strip() != _split_layout_usd_key(ext)
    ):
        if bool(getattr(ext, "_lam_startup_aux_load_inflight", False)):
            notify_sim_split_ui_sync(ext)
            return
        if bool(getattr(ext, "_lam_layout_first_aux_load_done", False)) and _split_layout_already_active(
            ext, n
        ):
            try:
                ext._lam_split_layout_usd_key = _split_layout_usd_key(ext)
    ...
    if (
        ...
        try:
            ...
            teardown_lam_multi_viewports(ext, skip_deferred_restore=True)
            try:
    ...
    if (
        n > 1
        and prev_n <= 1
        and not list(getattr(ext, "_lam_multi_viewport_entries", []) or [])
    ):
        for ti in range(1, 5):
...
def apply_sim_viewport_split_layout(ext: Any, split_n: int) -> None:
    ...
    _cancel_pending_split_viewport_restore(ext)
    tok = int(getattr(ext, "_lam_multi_viewport_apply_token", 0) or 0) + 1
    try:
        ext._lam_multi_viewport_apply_token = tok
    except Exception:
        pass
    _clear_viewport_layout_sync_caches()
    ...
    async def _deferred() -> None:
        await kit_app.get_app().next_update_async()
        if int(getattr(ext, "_lam_multi_viewport_apply_token", 0) or 0) != tok:
            return
...
def attach_stage_visibility_subscription(ext: Any, sync_fn: Callable[[], None]) -> None:
    ...
    try:
        ext._sim_split_stage_sub = ctx.get_stage_event_stream().create_subscription_to_pop(
            lambda _e: sync_fn(),
            name="morph.lam_control_1:sim_multi_split_visibility",
        )
    except Exception:
...
def _viewport_window_from_obj(obj: Any) -> Optional[Any]:
    """객체에서 ``get_frame`` 가능한 ViewportWindow 추출."""
    if obj is None:
        return None
    if callable(getattr(obj, "get_frame", None)):
        return obj
    for attr in ("viewport_window", "window", "_viewport_window", "_window", "viewport"):
        try:
...
def _resolve_viewport_window_from_split_entries(ext: Any, wname: str) -> Optional[Any]:
    """Dock 분할 시 ``_lam_multi_viewport_entries`` / 생성 캐시 의 ViewportWindow."""
    if ext is None:
        return None
    want = str(wname)
    try:
        ...
        if isinstance(cache, dict):
            vw = _viewport_window_from_obj(cache.get(want))
            if vw is not None:
                return vw
            # get_frame 없어도 캐시 객체 자체 반환 (overlay 가 viewport_api 로 폴백 가능)
            raw = cache.get(want)
            if raw is not None:
                return raw
    except Exception:
    ...
    for ent in entries:
        if not isinstance(ent, dict):
            continue
        if str(ent.get("win_name") or "") != want:
            continue
        for key in ("viewport_window", "window", "kit_vp"):
            raw = ent.get(key)
            vw = _viewport_window_from_obj(raw)
            if vw is not None:
                return vw
            if raw is not None:
                return raw
...
def _resolve_viewport_api_for_workspace_name(wname: str) -> Optional[Any]:
    """``LAM_SimSplit_*`` / ``Viewport`` 의 viewport_api (play·camera 용)."""
    try:
        api = _split_viewport_api(str(wname))
    except Exception:
        api = None
    if api is not None:
        # create_viewport_window 반환값이 곧 API 이거나 Window 인 경우
        inner = getattr(api, "viewport_api", None)
    ...
    try:
        ...
        ext = get_lam_extension_instance()
        vw = _resolve_viewport_window_from_split_entries(ext, str(wname))
        if vw is not None:
            inner = getattr(vw, "viewport_api", None)
            if inner is not None:
                return inner
            # create_viewport_window 결과가 API 자체인 경우
            if hasattr(vw, "usd_context_name") or hasattr(vw, "set_usd_context_name"):
                return vw
            if hasattr(vw, "camera_path") or hasattr(vw, "projection"):
                return vw
    except Exception:
        pass
    return None
...
def _resolve_viewport_window_for_workspace_name(wname: str) -> Optional[Any]:
    """
    ``Workspace`` 창 이름(예: ``Viewport``, ``LAM_SimSplit_1``)에 대응하는 ``ViewportWindow`` 를 찾는다.

    Dock: entries / create_viewport_window / Workspace / get_viewport_from_window_name.
    Widget: hud_mount.
    """
    try:
        from .lam_multi_viewport_widget import get_split_hud_mount, is_split_widget_layout_active
        from .lam_extension_singleton import get_lam_extension_instance
        ...
        if ext is not None and is_split_widget_layout_active(ext) and str(wname) != "Viewport":
            ...
            if mount is not None and callable(getattr(mount, "get_frame", None)):
                return mount
        # Dock: 생성 시 entries 에 넣은 ViewportWindow 가 가장 신뢰도 높음
        if ext is not None:
            from_ent = _resolve_viewport_window_from_split_entries(ext, str(wname))
    ...
    try:
        ...
        api = None
    if api is not None:
        vw = _viewport_window_from_obj(api)
        if vw is not None:
            return vw
    # Workspace 창 객체에서 ViewportWindow 추출
    try:
        import omni.ui as ui  # type: ignore

        ws = ui.Workspace.get_window(str(wname))
        vw = _viewport_window_from_obj(ws)
        if vw is not None:
            return vw
    except Exception:
        pass
