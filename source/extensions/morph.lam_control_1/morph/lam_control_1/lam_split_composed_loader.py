...
def resolve_split_master_usd_path(ext: Any) -> Optional[str]:
    ...
    raw = ""
    try:
        raw = str(getattr(ext, "_lam_last_loaded_usd_path", "") or "").strip()
    except Exception:
        raw = ""
    if raw:
...
def _main_stage_cache_key(ext: Any) -> str:
    try:
        ...
        try:
            ...
            rev = 0
        try:
            inst_n = len(getattr(ext, "_lam_registry", None).all_instances()) if getattr(ext, "_lam_registry", None) else 0
        except Exception:
            inst_n = 0
...
def get_or_export_main_composed_stage(ext: Any, token: int, tile_index: int = 0) -> Optional[str]:
    ...
    try:
        cached_key = str(getattr(ext, "_lam_split_composed_export_cache_key", "") or "")
        cached_tok = int(getattr(ext, "_lam_split_composed_export_token", -1) or -1)
        cached_path = str(getattr(ext, "_lam_split_composed_export_path", "") or "").strip()
        if (
            cache_key
    ...
    if path:
        try:
            ext._lam_split_composed_export_path = path
            ext._lam_split_composed_export_token = int(token)
            ext._lam_split_composed_export_cache_key = cache_key
        except Exception:
            pass
    return path
...
def clear_split_composed_export_cache(ext: Any) -> None:
    try:
        ext._lam_split_composed_export_path = None
        ext._lam_split_composed_export_token = None
        ext._lam_split_composed_export_cache_key = None
        ext._lam_split_composed_prewarm_inflight = False
    except Exception:
        pass
...
def ext_has_composed_instances(ext: Any) -> bool:
    ...
    try:
        reg = getattr(ext, "_lam_registry", None)
...
def composed_split_snapshot_ready(ext: Any) -> bool:
    ...
    try:
        cached_key = str(getattr(ext, "_lam_split_composed_export_cache_key", "") or "").strip()
        cached_path = str(getattr(ext, "_lam_split_composed_export_path", "") or "").strip()
...
async def resolve_composed_snapshot_for_split_async(
    ...
    *,
    ...
    """
    ready_path = str(getattr(ext, "_lam_split_composed_export_path", "") or "").strip()
    ...
    while waited < max(1, int(max_wait_frames)):
        if composed_split_snapshot_ready(ext):
            p = str(getattr(ext, "_lam_split_composed_export_path", "") or "").strip()
            if p and os.path.isfile(p):
                return p
        inflight = bool(getattr(ext, "_lam_split_composed_prewarm_inflight", False))
    ...
    if not bool(getattr(ext, "_lam_split_composed_prewarm_inflight", False)):
...
def schedule_split_composed_snapshot_prewarm(ext: Any) -> None:
    ...
    **빠르면서도 인스턴스 geometry 가 포함**된다.
    ...
    if composed_split_snapshot_ready(ext):
        return
    if bool(getattr(ext, "_lam_split_composed_prewarm_inflight", False)):
        return
    try:
        ext._lam_split_composed_prewarm_inflight = True
    except Exception:
        pass
...
def export_main_composed_stage_to_temp(ext: Any, token: int, tile_index: int) -> Optional[str]:
    ...
    try:
        ...
        path = os.path.normpath(
            os.path.join(
                tempfile.gettempdir(),
                f"morph_lam_composed_aux_{tile_index}_{token}_{os.getpid()}.usda",
            )
        )
...
def _runtime_map(ext: Any) -> Dict[str, SplitScreenRuntime]:
    m = getattr(ext, "_lam_split_runtime_by_screen", None)
    if not isinstance(m, dict):
        m = {}
        ext._lam_split_runtime_by_screen = m
    return m
...
def register_main_composed_runtime(ext: Any) -> None:
    ...
    try:
        ...
        master = getattr(win, "_master", None) if win is not None else None
        reg = getattr(ext, "_lam_registry", None)
        ev = getattr(ext, "_lam_evaluator", None)
        sch = getattr(ext, "_lam_scheduler", None)
...
def _refresh_aux_viewport_resolution(
    ...
    try:
        ...
        if sn > 1:
            try:
                from .lam_multi_viewport_widget import is_split_widget_layout_active, sync_split_widget_fill_frame, sync_split_widget_aux_render
            ...
            refresh_split_viewport_resolution_from_grid(
                wn, sn, ext=ext, force_window_rect=not bool(
                    getattr(ext, "_lam_split_used_dock_layout", False)
                ),
            )
            return
    except Exception:
...
def hydrate_split_screen_composed_stage(
    ...
    *,
    ...
    try:
        from . import lam_multi_viewport_diag as _mdiag
