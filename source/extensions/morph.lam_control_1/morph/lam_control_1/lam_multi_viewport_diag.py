...
Console 필터: ``[LAM/multi-diag]``

분할 뷰(N>1)에서만 기본 활성. 단일 화면에서도 강제하려면
``ext._lam_multi_viewport_diag_force = True`` 를 설정한다.
"""
...
def enabled(ext: Any = None) -> bool:
    ...
    if ext is not None:
        try:
            if bool(getattr(ext, "_lam_multi_viewport_diag_force", False)):
                return True
        except Exception:
            pass
        try:
...
def usd_context_for_screen(ext: Any, screen: int) -> Optional[str]:
    ...
    return f"morph_lam_split_aux_{s - 1}"
...
def runner_state_line(ext: Any, screen: int, runner: Any = None) -> str:
    ...
    try:
        ...
        pass
    reg_tag = _obj_tag(getattr(runner, "_lam_registry", None) if runner else None)
    sch_tag = _obj_tag(getattr(runner, "_lam_scheduler", None) if runner else None)
    return (
...
def motion_busy_line(ext: Any, screen: int) -> str:
    ...
    try:
        ...
        from .lam_translate_animation import count_translate_animations_for_context
