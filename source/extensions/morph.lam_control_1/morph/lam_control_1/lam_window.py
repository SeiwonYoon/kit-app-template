...
class LamWindow:
    ...
    def set_master_open_listener(self, fn: Optional[Callable[[], None]]) -> None:
        """open_master 성공 후 콜백 (듀얼 viewport·화면2 USD 로드 등)."""
        self._master_open_listener = fn

    def _csv_play_screen_count(self) -> int:
        from .lam_sim_control_defaults import default_csv_play_screen_count

        return int(default_csv_play_screen_count())

    def _primary_csv_sim_window(self) -> Optional[LamSimulationCsvPlayWindow]:
        """화면1 CSV 재생창 — HUD·오버레이 SSOT."""
        return self._csv_sim_windows.get(1)

    def _ensure_csv_sim_play_window(self, screen: int) -> LamSimulationCsvPlayWindow:
        si = max(1, int(screen))
        win = self._csv_sim_windows.get(si)
        reg, sch = self._registry_scheduler_for_csv_screen(si)
        if win is None:
            win = LamSimulationCsvPlayWindow(
                reg,
                sch,
                screen=si,
            )
            win.set_lam_window(self)
            self._csv_sim_windows[si] = win
        else:
            win._registry = reg
            win._scheduler = sch
        return win

    def _registry_scheduler_for_csv_screen(self, screen: int) -> tuple[Any, Any]:
        from .lam_csv_play_screen import get_registry_scheduler_for_lam_screen

        si = int(screen)
        return get_registry_scheduler_for_lam_screen(
            self,
            si,
            allow_fallback=(si <= 1),
        )

    def refresh_csv_sim_play_runtimes(self) -> None:
        """분할 USD 로드 후 화면별 registry/scheduler 를 재생창에 반영."""
        for si in list(self._csv_sim_windows.keys()):
            try:
                reg, sch = self._registry_scheduler_for_csv_screen(si)
                win = self._csv_sim_windows.get(si)
                if win is not None:
                ...
                pass
        for si in range(2, self._csv_play_screen_count() + 1):
            try:
                self.sync_csv_viewport_overlays_for_screen(si)
            except Exception:
                pass

    def _on_hud_csv_play_clicked(self) -> None:
        """HUD 재생 — 화면1·화면2 동시 Play (각 창 설정·registry 사용)."""
        self._ensure_csv_sim_play_window(1)._on_play_clicked()
        if self._csv_play_screen_count() >= 2:
            self._ensure_csv_sim_play_window(2)._on_play_clicked()

    def _on_hud_csv_pause_clicked(self) -> None:
        """HUD 일시정지 — 활성 화면별 세션 일시정지."""
        for si in range(1, self._csv_play_screen_count() + 1):
            win = self._csv_sim_windows.get(si)
            if win is not None and win._csv_play_thread_alive():
                win._on_csv_pause_clicked()

    def _on_hud_csv_stop_reset_clicked(self) -> None:
        """HUD 정지(초기화) — 화면별 정지."""
        for si in range(1, self._csv_play_screen_count() + 1):
            win = self._csv_sim_windows.get(si)
            if win is not None:
                win._on_csv_stop_reset_clicked()

    def sync_csv_viewport_overlays_for_screen(self, screen: int) -> None:
        """화면별 CSV 창 설정 → 해당 화면 viewport/USD 만 동기화 (``lam_csv_screen_runtime``)."""
        try:
            from .lam_csv_screen_runtime import sync_csv_screen_overlays

            sync_csv_screen_overlays(self, int(screen))
        except Exception as exc:
            print(
                f"{_PRINT_PREFIX} sync_csv_viewport_overlays screen={screen}: {exc}",
                flush=True,
            )

    def _sync_aux_kit_window_visibility(self) -> None:
        ext = self._kit_ext
        if ext is None:
            return
        try:
    ...
    def _show_all_csv_sim_play_windows(self) -> None:
        ...
        for si in range(1, self._csv_play_screen_count() + 1):
            try:
                if ext is not None:
                    from .lam_aux_kit_window_ui import ensure_csv_play_screen_model

                    ensure_csv_play_screen_model(ext, si)
                self._ensure_csv_sim_play_window(si).show()
            except Exception as exc:
                print(
                    f"{_PRINT_PREFIX} CSV 재생창 화면{si} show 실패: {exc}",
                    flush=True,
                )
        self._sync_aux_kit_window_visibility()
    ...
    def show(self) -> None:
                    ...
                    with ui.VStack(spacing=4):
                        with ui.HStack(spacing=4, height=24):
                            ...
                            ui.Button(
                                ...
                                width=160,
                                tooltip="이미 떠 있는 화면별 CSV 재생창을 앞으로 가져옵니다",
                            )
                            ui.Spacer()
                        with ui.HStack(spacing=4, height=24):
        ...
        self._show_all_csv_sim_play_windows()
        self._sync_aux_kit_window_visibility()
        self._sync_csv_viewport_hud()
        self._sync_wafer_foup_viewport_labels_only()
    ...
    def _sync_csv_viewport_hud(self) -> None:
        ...
        - ``SHOW_VIEWPORT_CSV_PANEL_TOGGLE_HOTSPOT``: Federation HUD 아래 투명 토글 버튼
        """
    ...
    def destroy(self) -> None:
        ...
        for panel in list(self._foup_status_3d_by_screen.values()):
            try:
                panel.destroy()
            except Exception:
                pass
        self._foup_status_3d_by_screen.clear()
        try:
            if self._device_labels_3d is not None:
                self._device_labels_3d.destroy()
        except Exception:
        ...
        for panel in list(self._device_labels_3d_by_screen.values()):
            try:
                panel.destroy()
            except Exception:
                pass
        ...
        for panel in list(self._wafer_foup_labels_by_screen.values()):
            try:
                ...
                pass
        self._wafer_foup_labels_by_screen.clear()
        try:
    ...
    def _schedule_autoload_master_on_startup(self) -> None:
        ...
        try:
            ...
            self._autoload_sub = stream.create_subscription_to_pop(
                _do, name="morph.lam_control_1.lam_window.autoload_master"
            )
        except Exception as exc:
            print(
    ...
    def _try_autoload_master_on_startup(self, *, layout_ready: bool = False) -> None:
        ...
        if ext is not None and not layout_ready:
            ...
            try:
                ...
                if lam_multi_viewport.preserve_split_layout_during_startup(ext):
                    return
            except Exception:
                pass
    ...
    def _schedule_instances_ui_refresh(self) -> None:
        ...
        try:
            ...
            self._inst_refresh_sub = stream.create_subscription_to_pop(
                _do, name="morph.lam_control_1.lam_window.instances_refresh"
            )
        except Exception:
            self._inst_refresh_pending = False
            self._refresh_instances()
