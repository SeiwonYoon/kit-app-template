...
실제 LightRig 복제·fallback DomeLight 제거는 `lam_multi_viewport_widget.py` 쪽이 담당하고,
본 모듈은 주로 진단 + (옵션) session TransferContent 실험용이다.

로그 prefix
-----------
  [LAM/p0b-api]          ViewportAPI 속성 dump
  [LAM/p0b-api-diff]     main vs aux 의미 있는 API 차이만
  [LAM/p0b-carb]         carb.settings tree dump
  [LAM/p0b-session]      root/session layer identifier·rootPrims
  [LAM/p0b-light]        UsdLux 조명 목록·Dome intensity
  [LAM/p0b-light-timing] 프레임 +1/+2/+5 Lux 타이밍
  [LAM/p0b-rp]           RenderProduct / Hydra 요약
  [LAM/p0b-introspect]   Kit viewport 관련 모듈 callable 목록
  [LAM/p0b-active]       get_active_viewport() 변화 추적
...
----------------------------
  [LAM/p0b-ord]   함수 호출 순서 (frame/ts/lux/api/rp)
  [LAM/p0b-frame] 120프레임 lux·tone·carb dump
  [LAM/p0b-who]   TBS_DefaultDomeLight 생성 stack
  [LAM/p0b-rs]    RenderSettings / Environment dump
"""
...
def p0b_fix_enabled() -> bool:
    ...
    (실사용 LightRig 복제는 lam_multi_viewport_widget 의 clone 경로가 담당.)
    """
...
def dump_viewport_api(api: Any, label: str) -> List[str]:
    ...
    for name in sorted(set(dir(api)) if api is not None else ()):
        ...
        if isinstance(value, (list, tuple)) and len(value) > 32:
            continue
        lines.append(f"[LAM/p0b-api] {label} {name}={_fmt(value)}")
    return lines
...
def dump_viewport_api_diff(main_api: Any, aux_api: Any) -> List[str]:
    ...
    prefix: [LAM/p0b-api-diff]
    """
    lines: List[str] = []
    if main_api is None or aux_api is None:
        return ["[LAM/p0b-api-diff] api=None"]
    ...
    for k in _P0B_API_DIFF_KEYS:
        ...
        try:
            ...
            same = False
        if not same:
            lines.append(f"[LAM/p0b-api-diff] {k} main={_fmt(mv)} aux={_fmt(av)}")
    ...
    for k in common:
        ...
        if not same:
            ...
            extra_count += 1
            if extra_count >= 80:
                lines.append("[LAM/p0b-api-diff] ... truncated (too many diffs)")
                break
...
def dump_carb_settings(prefixes: Sequence[str], *, label: str) -> List[str]:
    ...
    try:
        ...
        return [f"[LAM/p0b-carb] {label} err={exc}"]
    ...
    while stack:
        ...
        try:
            ...
            val = None
        lines.append(f"[LAM/p0b-carb] {label} {path}={_fmt(val)}")
...
def dump_session_layers(*, label: str, ctx_name: str) -> List[str]:
    ...
    prefix: [LAM/p0b-session]
    """
    lines: List[str] = []
    try:
        from pxr import Sdf  # noqa: F401
    except Exception as exc:
        return [f"[LAM/p0b-session] {label} ctx={ctx_name!r} pxr_err={exc}"]

    st = _stage_for_ctx(ctx_name)
    if st is None:
        return [f"[LAM/p0b-session] {label} ctx={ctx_name!r} stage=None"]
    try:
        root = st.GetRootLayer()
        sess = st.GetSessionLayer()
        lines.append(
            f"[LAM/p0b-session] {label} ctx={ctx_name!r} root={getattr(root, 'identifier', None)!r} "
        ...
        for lyr, nm in ((root, "root"), (sess, "session")):
            ...
            try:
                ...
                sub = []
            lines.append(f"[LAM/p0b-session] {label} {nm}.sublayers={sub!r}")
            ...
            try:
                ...
                pp = []
            if pp:
                lines.append(f"[LAM/p0b-session] {label} {nm}.rootPrims={pp!r}")
    except Exception as exc:
        lines.append(f"[LAM/p0b-session] {label} ctx={ctx_name!r} err={exc}")
    return lines
...
def dump_usdlux(*, label: str, ctx_name: str) -> Tuple[List[str], List[str]]:
    ...
    prefix: [LAM/p0b-light]
    """
    lines: List[str] = []
    st = _stage_for_ctx(ctx_name)
    if st is None:
        return ([f"[LAM/p0b-light] {label} ctx={ctx_name!r} stage=None"], [])
    try:
        from pxr import UsdLux
    except Exception as exc:
        return ([f"[LAM/p0b-light] {label} ctx={ctx_name!r} pxr_err={exc}"], [])
    paths: List[str] = []
    ...
    try:
        ...
        for prim in st.Traverse():
            try:
                ...
                continue
        lines.append(f"[LAM/p0b-light] {label} ctx={ctx_name!r} light_count={len(paths)}")
        if paths:
            lines.append(f"[LAM/p0b-light] {label} ctx={ctx_name!r} lights={paths!r}")
            try:
                lines.append(f"[LAM/p0b-light] {label} ctx={ctx_name!r} light_types={typed!r}")
            except Exception:
                pass
        for p in paths:
            ...
            try:
                ...
                if dome:
                    ...
                    lines.append(
                        f"[LAM/p0b-light] {label} dome={p} texture={tex!r} intensity={inten!r}"
                    )
            except Exception:
                pass
    except Exception as exc:
        lines.append(f"[LAM/p0b-light] {label} ctx={ctx_name!r} err={exc}")
    return (lines, paths)
...
def dump_rp_hydra(api: Any, *, label: str) -> List[str]:
    ...
    lines: List[str] = []
    if api is None:
        return [f"[LAM/p0b-rp] {label} api=None"]
    attrs = (
        "render_mode",
        "hydra_engine",
        "resolution",
    ...
    for a in attrs:
        try:
            lines.append(f"[LAM/p0b-rp] {label} api.{a}={_fmt(getattr(api, a, None))}")
        except Exception:
            pass
    for m in ("get_render_product_path", "get_render_product", "get_render_settings", "get_renderer"):
        ...
        if callable(fn):
            try:
                lines.append(f"[LAM/p0b-rp] {label} api.{m}()={_fmt(fn())}")
            except Exception as exc:
                lines.append(f"[LAM/p0b-rp] {label} api.{m}()=err:{exc}")
    return lines
...
def dump_viewport_modules_introspection() -> List[str]:
    ...
    def _list_callables(mod: Any, *, label: str) -> None:
        ...
        try:
            ...
            lines.append(
                f"[LAM/p0b-introspect] {label} callables_count={len(names)} show={show!r}"
            )
        except Exception as exc:
            lines.append(f"[LAM/p0b-introspect] {label} err={exc}")
    ...
    try:
        ...
        lines.append(
            f"[LAM/p0b-introspect] omni.kit.viewport.utility import_err={exc}"
        )
    ...
    for mod_name in (
        ...
        try:
            ...
            lines.append(f"[LAM/p0b-introspect] {mod_name} import_err={exc}")
...
def run_p0b_diagnostics(
    *,
    ...
    aux_api: Any,
    main_ctx: str = "",
    aux_ctx: str = "morph_lam_split_aux_1",
) -> None:
    """
    P0-B 진단 오케스트레이터 (동기).
...
def run_p0b_fix_if_needed(
    *,
    ...
    `lam_multi_viewport_widget._clone_default_light_rig_from_main_to_aux` 가 담당.
    ...
    try:
        print(
            f"[LAM/p0b-session] fix_applied={bool(ok)} main_ctx={main_ctx!r} aux_ctx={aux_ctx!r}",
            flush=True,
        )
    except Exception:
        pass
...
async def run_p0b_light_timing_and_active_tracker(
    *,
    ...
    prefix: [LAM/p0b-active], [LAM/p0b-light-timing]
    """
    if not p0b_diag_enabled():
        return
    try:
    ...
    try:
        print(
            f"[LAM/p0b-active] tracker_start main_ctx={main_ctx!r} aux_ctx={aux_ctx!r} frames={list(frames)!r}",
            flush=True,
        )
    except Exception:
        pass
    ...
    while f <= max_f:
        ...
        if cur != last_active:
            ...
            try:
                print(
                    f"[LAM/p0b-active] frame={f} id={snap.get('id')!r} "
        ...
        if f in targets:
            try:
                ...
                for ln in main_lines + aux_lines:
                    print(
                        f"[LAM/p0b-light-timing] "
                        f"{ln[len('[LAM/p0b-light] '):] if ln.startswith('[LAM/p0b-light] ') else ln}",
                        flush=True,
                    )
            except Exception as exc:
                try:
                    print(f"[LAM/p0b-light-timing] frame={f} err={exc}", flush=True)
                except Exception:
                    pass
...
# Logs: [LAM/p0b-ord] [LAM/p0b-frame] [LAM/p0b-who] [LAM/p0b-rs]
# =============================================================================
...
def log_p0b_call_order(
    ...
    *,
    ...
    try:
        ...
        print(
            f"[LAM/p0b-ord] step={step!r} frame={frame} ts={ts:.3f} "
        ...
        try:
            print(f"[LAM/p0b-ord] step={step!r} err={exc}", flush=True)
        except Exception:
            pass
...
def log_fallback_create_stack(*, ctx_name: str, light_path: str) -> None:
    ...
    try:
        ...
        print(
            f"[LAM/p0b-who] CREATE {light_path!r} ctx={ctx_name!r} "
            f"frame={_kit_frame_number()} ts={time.time():.3f}\n{stack}",
            flush=True,
        )
    except Exception as exc:
        try:
            print(f"[LAM/p0b-who] CREATE err={exc}", flush=True)
        except Exception:
            pass
...
def dump_render_settings(*, label: str, ctx_name: str) -> List[str]:
    ...
    st = _stage_for_ctx(ctx_name)
    if st is None:
        return [f"[LAM/p0b-rs] {label} ctx={ctx_name!r} stage=None"]
    try:
        ...
        for prim in st.Traverse():
            try:
                ...
                if tname in ("RenderSettings", "RenderProduct", "RenderVar") or (
                    ...
                    if attrs_out:
                        lines.append(
                            f"[LAM/p0b-rs] {label} {path} attrs={attrs_out!r}"
                        )
            except Exception:
                continue
        lines.insert(
            0,
            f"[LAM/p0b-rs] {label} ctx={ctx_name!r} prims={interesting[:40]!r}",
        )
    except Exception as exc:
        lines.append(f"[LAM/p0b-rs] {label} err={exc}")
    return lines
...
def dump_tone_carb_snapshot(*, label: str) -> List[str]:
    ...
    try:
        ...
        return [f"[LAM/p0b-frame] {label} carb_err={exc}"]
    for path in _carb_tone_keys():
        try:
            lines.append(f"[LAM/p0b-frame] {label} carb {path}={_fmt(s.get(path))}")
        except Exception:
            lines.append(f"[LAM/p0b-frame] {label} carb {path}=<?>")
    return lines
...
async def run_p0b_frame_tone_tracker(
    *,
    ...
    print(
        f"[LAM/p0b-frame] tracker_start frames={n} main_ctx={main_ctx!r} aux_ctx={aux_ctx!r} "
    ...
    for f in range(n):
        ...
        line = (
            f"[LAM/p0b-frame] f={f}/{n-1} kit={_kit_frame_number()} "
        ...
        if first_diff_frame is None and not same_lux:
            first_diff_frame = f
            print(
                f"[LAM/p0b-frame] FIRST_LUX_DIFF f={f} kit={_kit_frame_number()} "
                f"main_lux={main_lux!r} aux_lux={aux_lux!r}",
                flush=True,
            )
    ...
    print(
        f"[LAM/p0b-frame] tracker_end first_lux_diff_frame={first_diff_frame!r} "
...
def install_fallback_who_notice(*, ctx_name: str, light_path: str = "/World/TBS_DefaultDomeLight") -> None:
    ...
    try:
        ...
        try:
            print(f"[LAM/p0b-who] notice_install_err={exc}", flush=True)
        except Exception:
            pass
        return
    ...
    def _on_objects_changed(notice, sender) -> None:  # noqa: ANN001
        try:
            ...
            print(
                f"[LAM/p0b-who] NOTICE path_hit={target!r} ctx={ctx_name!r} "
                f"frame={_kit_frame_number()} ts={time.time():.3f} "
            ...
            try:
                print(f"[LAM/p0b-who] NOTICE err={exc}", flush=True)
            except Exception:
                pass
    ...
    try:
        ...
        print(
            f"[LAM/p0b-who] notice_installed ctx={ctx_name!r} path={target!r} "
            f"frame={_kit_frame_number()}",
            flush=True,
        )
    except Exception as exc:
        try:
            print(f"[LAM/p0b-who] notice_install_fail={exc}", flush=True)
        except Exception:
            pass
...
def schedule_p0b_frame_tone_tracker_once(
    *,
    ...
    if _frame_tracker_once:
        try:
            print(f"[LAM/p0b-frame] skip_already_started label={label!r}", flush=True)
        except Exception:
            pass
        return False
    ...
    try:
        print(
            f"[LAM/p0b-frame] schedule_once label={label!r} frames={frames} "
            f"kit={_kit_frame_number()}",
            flush=True,
        )
        # Kit IApp has no create_task — use asyncio (same pattern as lam_multi_viewport).
        ...
        _frame_tracker_once = False
        try:
            print(f"[LAM/p0b-frame] schedule_fail label={label!r} err={exc}", flush=True)
        except Exception:
            pass
        return False
