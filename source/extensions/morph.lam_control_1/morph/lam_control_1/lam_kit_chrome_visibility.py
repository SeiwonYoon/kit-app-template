...
LAM Multi-USD Load·시퀀스 편집기·Viewport·멀티 시뮼 분할 보조 창(``LAM_SimSplit_*``)은 숨기지 않는다.
...
【시뮼 분할·제어창과의 연동】
- 메뉴/툴바 등 표시 상태가 바뀌면 제어창 쪽에서 ``lam_multi_viewport.schedule_split_layout_refresh_for_chrome_change`` 를 호출해
...
def apply_viewport_dock_tab_bars_hidden() -> None:
    """Viewport·LAM_SimSplit_* 의 Dock 탭 바(뷰포트 1/2·화면 탭) 숨김."""
    seen: Set[str] = set()
    for w in _iter_workspace_windows():
        label = _window_label(w)
    ...
    for name in ("Viewport", "LAM_SimSplit_1", "LAM_SimSplit_2", "LAM_SimSplit_3"):
        try:
            w = ui.Workspace.get_window(name)
            if w is not None:
                _set_dock_tab_bar_hidden_on_window(w)
