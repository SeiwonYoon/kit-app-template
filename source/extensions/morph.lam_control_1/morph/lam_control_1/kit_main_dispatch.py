...
TBS ``morph.tbs_control_2.kit_main_dispatch`` 와 동일 구조:
update 이벤트 스트림 구독이 매 프레임 메인 스레드에서 큐를 비운다.
(``IApp.post_update`` 는 존재하지 않는 API — 사용 금지.)
"""
...
def ensure_kit_main_dispatch() -> None:
    ...
    try:
        _update_sub = kit_app.get_app().get_update_event_stream().create_subscription_to_pop(
            _pump_main_queue,
            name="morph.lam_control_1:kit_main_dispatch",
        )
    except Exception:
        _update_sub = None
