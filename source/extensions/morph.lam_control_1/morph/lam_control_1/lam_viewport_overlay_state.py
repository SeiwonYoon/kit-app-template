...
_foup_counts_by_screen: Dict[int, Dict[int, FoupCounts]] = {
    1: dict(_foup_counts),
}
...
def _ensure_foup_screen(screen: int) -> Tuple[Dict[int, FoupCounts], set[Tuple[Any, ...]]]:
    si = max(1, int(screen))
    counts = _foup_counts_by_screen.get(si)
    if counts is None:
        t = 25
        counts = {
        ...
        _foup_counts_by_screen[si] = counts
    keys = _foup_counted_schedule_keys_by_screen.get(si)
    if keys is None:
        keys = set()
        _foup_counted_schedule_keys_by_screen[si] = keys
    if si == 1:
        _foup_counts.update(counts)
    return counts, keys
    ...
    *,
    ...
    try:
        ...
        _play_prim_hide_retry_sub = stream.create_subscription_to_pop(
            _tick,
            name="morph.lam_control_1.play_prim_hide.startup_retry",
        )
    except Exception as exc:
        print(
...
def set_toggle_foup_status(enabled: bool) -> None:
    ...
    if not bool(enabled):
        # 화면1 전역 토글만 — 화면2+ 패널은 절대 건드리지 않음
        try:
            from .lam_viewport_foup_status_3d import force_remove_foup_sceneviews

            force_remove_foup_sceneviews(screen=1)
            print("[LAM/OverlayState] force_remove_foup_sceneviews(screen=1)", flush=True)
        except Exception:
            pass
...
def set_toggle_device_labels(enabled: bool) -> None:
    ...
    if not bool(enabled):
        # 화면1 전역 토글만 — 화면2+ 패널은 절대 건드리지 않음
        try:
            from .lam_viewport_device_labels_3d import force_remove_device_sceneviews

            force_remove_device_sceneviews(screen=1)
            print("[LAM/OverlayState] force_remove_device_sceneviews(screen=1)", flush=True)
        except Exception:
            pass
    _notify_toggle_listeners()
    _sync_ui_model_values()
...
def set_foup_counts(
    ...
    *,
    ...
    if fi not in (1, 2, 3):
        return
    with _lock:
        counts, _ = _ensure_foup_screen(screen)
        counts[fi] = FoupCounts(
            total=int(total),
            picked_count=int(picked),
            placed_back_count=int(placed_back),
        )
        if int(screen) == 1:
            _foup_counts[fi] = counts[fi]


def get_foup_counts(foup_index: int, *, screen: int = 1) -> FoupCounts:
    fi = int(foup_index)
    with _lock:
        counts, _ = _ensure_foup_screen(screen)
    ...
    *,
    ...
    with _lock:
        ...
        if po == "pick":
            ...
            return False
        if si == 1:
            _foup_counts[foup_n] = counts[foup_n]
            _foup_counted_schedule_keys.add(row_key)
    notify_foup_counts_ui_refresh(screen=si)
    return True


def notify_foup_counts_ui_refresh(*, screen: Optional[int] = None) -> None:
