...
_ISOLATED_TEST_TAG = "[TBS/rp-isolated]"


def viewport_rp_diag_enabled() -> bool:
    try:
        from .sim_control_defaults import VIEWPORT_RP_DIAG_ENABLED
...
def viewport_rp_isolated_test_enabled() -> bool:
    try:
        from .sim_control_defaults import VIEWPORT_RP_ISOLATED_WINDOW_TEST
...
def viewport_rp_timeline_frames() -> int:
    try:
        from .sim_control_defaults import VIEWPORT_RP_TIMELINE_FRAMES
...
def log_rp_investigation(
    ...
    *,
    ...
    """구조화된 다줄 로그 — ``[TBS/rp-invest]`` 접두사."""
    if not viewport_rp_diag_enabled():
        return {}
    probe = probe_render_product_chain(api, widget, scene_view=scene_view)
    try:
        print(f"[TBS/rp-invest] ===== phase={phase!r} label={label!r} =====", flush=True)
        print(
            f"[TBS/rp-invest] HAS_RENDER_PRODUCT={probe.get('HAS_RENDER_PRODUCT', '?')} "
        ...
        for group_name, keys in groups:
            ...
            if lines:
                print(f"[TBS/rp-invest] -- {group_name} --", flush=True)
                for line in lines:
                    print(f"[TBS/rp-invest]{line}", flush=True)
        if extra:
            print(f"[TBS/rp-invest] extra: {extra}", flush=True)
        print(f"[TBS/rp-invest] ===== end {phase!r} =====", flush=True)
    except Exception:
        pass
    return probe
...
async def observe_rp_timeline(
    ...
    *,
    ...
    try:
        print(
            f"[TBS/rp-timeline] START label={label!r} frames={n_frames}",
            flush=True,
        )
    except Exception:
        pass
    for frame_i in range(n_frames):
        ...
        try:
            print(
                f"[TBS/rp-timeline] frame={frame_i + 1}/{n_frames} label={label!r} "
            ...
            pass
    try:
        print(f"[TBS/rp-timeline] END label={label!r}", flush=True)
    except Exception:
        pass
...
async def run_isolated_viewport_widget_test(
    ...
    *,
    ...
    try:
        ext._tbs_rp_isolated_test_window = win
        ext._tbs_rp_isolated_test_widget = vw
        ext._tbs_rp_isolated_test_result = result
    except Exception:
        pass
    return result
...
def teardown_isolated_rp_test_window(ext: Any = None) -> None:
    ...
    if ext is not None:
        try:
            vw = getattr(ext, "_tbs_rp_isolated_test_widget", None)
            win = getattr(ext, "_tbs_rp_isolated_test_window", None)
        except Exception:
            vw = None
            win = None
    ...
    if ext is not None:
        try:
            ext._tbs_rp_isolated_test_window = None
            ext._tbs_rp_isolated_test_widget = None
        except Exception:
            pass
...
def log_finalize_rp_step(aux_rec: Any, step: str) -> None:
    ...
    if not isinstance(aux_rec, dict):
        return
    wn = str(aux_rec.get("_win_name") or "TBS_SimSplit_1")
    api = aux_rec.get("api")
    ...
    try:
        print(
            f"[TBS/rp-finalize] step={step!r} tile={wn!r} "
