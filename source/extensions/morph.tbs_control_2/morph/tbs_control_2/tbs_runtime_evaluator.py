...
from .tbs_attribute_reauthor import AttributeReauthorCache
from .tbs_instance_registry import AnimationInstanceRegistry
from .tbs_instance_runtime import AnimationInstanceRuntime
from .tbs_types import AnimationInstance, TBS_FIXED_FPS
...
class RuntimeEvaluator:
    ...
    def set_snap_timecode_to_frame(self, enabled: bool) -> None:
        """timeSamples / Option E 평가 시 ``round(vt * TBS_FIXED_FPS)`` 로 정수 timeCode 만 사용."""
    ...
    def dump_master_timesamples_usda(self, prim_path: str):
        ...
        from .tbs_asset_diagnostics import _decide_kind
        ...
        from .tbs_types import ASSET_KIND_UNKNOWN, AssetDiag
            ...
            try:
                ...
                if not getattr(inst, "_lam_diag_reauthor_seen", False):
                    ...
                    print(
                        ...
                        f"v_t={new_t:.3f}s tps={TBS_FIXED_FPS}(forced) "
                        f"timeCode={(new_t + float(inst.offset_sec)) * TBS_FIXED_FPS:.3f} "
                        f"wrote={wrote}",
                        flush=True,
                    )
                    if wrote == 0:
    ...
    TBS_FIXED_FPS: float = 30.0

    def _stage_tps(self, stage) -> float:  # noqa: ARG002
        return self.TBS_FIXED_FPS

    def _ensure_stage_fps_lam_fixed(self, stage) -> None:
        """master stage 의 timeCodesPerSecond / framesPerSecond 를 TBS_FIXED_FPS 로 강제.
        ...
        if stage is None or getattr(self, "_diag_stage_fps_done", False):
            return
        target = float(self.TBS_FIXED_FPS)
        try:
                    ...
                    try:
                        ...
                        for i, spec in enumerate(stack):
                            try:
                                ...
                                layer_id = "?"
                            tag = "[TBS]" if "lam_inst_" in layer_id else ""
                            try:
                                refs = (
                    ...
                    if p and p.IsValid():
                        ...
                        for j, spec_after in enumerate(prim_stack_after):
                            try:
                                ...
                                lid = "?"
                            tag2 = "[TBS]" if "lam_inst_" in lid else ""
