...
【 호출 모델 】
- `TbsLamSequenceRunner.run(steps)` 은 동기 차단형. background thread 에서 호출 권장
...
from . import tbs_lam_translate_animation as _ltx_preload  # type: ignore  # noqa: E402,F401
from . import tbs_lam_rotate_animation as _lrx_preload  # type: ignore  # noqa: E402,F401
...
def _refresh_instance_asset_time_from_stage(instance) -> tuple[float, float, float]:
    ...
    if mn is not None and mx is not None and mx > mn:
        # FPS 30 고정 정책 — tps 는 항상 TBS_FIXED_FPS.
        tps = TBS_FIXED_FPS
        try:
...
def _playback_speed_scale(fallback: float) -> float:
    ...
    try:
        from .tbs_sim_play_stubs import (
...
def _csv_play_nominal_sleep(runner: "TbsLamSequenceRunner", wall_sec: float, *, allow_stop: bool) -> None:
...
class TbsLamSequenceRunner:
    ...
    def stop(self, *, cancel_all_move_rotate: bool = True) -> None:
        ...
        Args:
            cancel_all_move_rotate: True(기본) — 시퀀스 편집기 Stop 과 동일하게 전역
                ...
                `stop_all_*` 는 호출하지 않는다. `tbs_json_test_unused` 가 동일
    ...
    def _wait_for_motion_complete(
        ...
        *,
        ...
        while not self._stop_flag.is_set():
            try:
                from .tbs_sim_play_stubs import sync_csv_play_live_speed_from_ui
    ...
    def _start_usd_timeline(
        ...
        if step_kind_label == STEP_KIND_USD_TIMELINE and not loop:
            ...
            def _snap_tl() -> None:
                from .tbs_master_timeline_play import snapshot_timeline
            ...
            def _tl_begin() -> None:
                ...
                from .tbs_master_timeline_play import begin_play_frame_range
            ...
            end_time_sec = float(play_ef) / float(TBS_FIXED_FPS)
            end_tc = float(play_ef)

            def _tl_end() -> None:
                from .tbs_master_timeline_play import (
                    end_play_pause,
    ...
    def _start_set_prim_visibility(self, idx: int, step: dict, speed_scale: float) -> float:
        ...
        if label_ctx:
            try:
                from .tbs_wafer_viewport_labels import (
