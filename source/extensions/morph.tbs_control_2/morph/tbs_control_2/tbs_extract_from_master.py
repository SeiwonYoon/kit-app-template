...
def _scan_layer_stats(layer: Any) -> dict:
    ...
    try:
        from .tbs_asset_diagnostics import _classify_attr_name, _is_omnigraph_prim
    except Exception:
...
def extract_subtree_to_anonymous_layer(
    ...
    *,
    ...
    #    offscreen stage 가 일관된 30fps 도메인에서 평가되도록 한다 (TBS_FIXED_FPS 정책).
    ...
    try:
        anon.framesPerSecond = float(TBS_FIXED_FPS)
        anon.timeCodesPerSecond = float(TBS_FIXED_FPS)
    except Exception:
        pass
    try:
