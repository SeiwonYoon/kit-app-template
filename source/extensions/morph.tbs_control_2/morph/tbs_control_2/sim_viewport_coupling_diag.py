...
def coupling_diag_enabled() -> bool:
    try:
        from .sim_control_defaults import VIEWPORT_COUPLING_DIAG_ENABLED
...
def snapshot_both_cameras(ext: Any, label: str = "") -> Dict[str, Any]:
    ...
    try:
        from .sim_multi_view_widget import _tile_usd_context_name
...
def log_manipulator_activation_state(ext: Any, active_wn: str, reason: str = "") -> None:
    ...
    if not coupling_diag_enabled():
        return
    try:
        from .sim_multi_view_widget import _tile_win_name, is_split_widget_layout_active
        ...
        print(
            f"[TBS/coupling-trace] MANIP_ACTIVATE reason={reason!r} active={active_wn!r} "
            + " | ".join(parts),
            flush=True,
        )
    except Exception:
...
def log_orbit_context_snapshot(ext: Any, usd_label: str) -> None:
    ...
    try:
        ...
        if not is_split_widget_layout_active(ext):
            return
        active_wn = str(getattr(ext, "_tbs_active_widget_tile", "") or "Viewport")
        native = _get_native_viewport_api()
        our_ids = _our_widget_tile_api_ids(ext)
        print(
            f"[TBS/coupling-trace] ORBIT_CTX usd_label={usd_label!r} "
...
def log_stage_lighting_summary(ext: Any, label: str = "") -> None:
    ...
    try:
        ...
        print(
            f"[TBS/coupling-report] stage-lux {label!r} "
            f"main_count={len(main_lux)} aux_count={len(aux_lux)}",
            flush=True,
        )
        if main_lux != aux_lux:
            print(
                f"[TBS/coupling-report]  main_lux={main_lux!r} aux_lux={aux_lux!r}",
                flush=True,
            )
    except Exception as exc:
        try:
            print(f"[TBS/coupling-report] stage-lux err={exc}", flush=True)
        except Exception:
            pass
...
def log_manipulator_investigation(ext: Any, phase: str = "READY") -> None:
    ...
    try:
        ...
        print(f"[TBS/coupling-report] ===== phase={phase!r} =====", flush=True)
        ...
        if native_api is not None:
            for attr in ("enable_input", "inputs_enabled", "camera_path"):
                try:
                    print(
                        f"[TBS/coupling-report]  native.{attr}={getattr(native_api, attr, None)!r}",
                        flush=True,
                    )
                except Exception:
                    pass
        ...
        try:
            ...
            print(f"[TBS/coupling-report] ViewportWidget.get_instances count={len(inst)}", flush=True)
            for i, w in enumerate(inst):
                ctx = getattr(w, "usd_context_name", None)
                vapi = getattr(w, "viewport_api", None)
                print(
                    f"[TBS/coupling-report]  #{i} widget={_fmt_id(w)} "
                    f"ctx={ctx!r} api={_fmt_id(vapi)}",
                    flush=True,
                )
        except Exception as exc:
            print(f"[TBS/coupling-report] get_instances err={exc}", flush=True)

        try:
            from omni.kit.viewport.utility import get_active_viewport

            active = get_active_viewport()
            print(f"[TBS/coupling-report] get_active_viewport()={_fmt_id(active)}", flush=True)
        except Exception as exc:
            print(f"[TBS/coupling-report] get_active_viewport err={exc}", flush=True)

        snap = snapshot_both_cameras(ext, phase)
        print(
            f"[TBS/coupling-report] main_cam={snap.get('main_cam_pos')} "
            f"aux_cam={snap.get('aux_cam_pos')}",
            flush=True,
        )
        print(f"[TBS/coupling-report] ===== end {phase!r} =====", flush=True)
    except Exception as exc:
        try:
            print(f"[TBS/coupling-report] FAIL {exc}", flush=True)
        except Exception:
            pass
...
class _CameraChangeTracker:
    ...
    def install(self) -> None:
        ...
        try:
            ...
            try:
                print("[TBS/coupling-trace] UsdNotice installed for main+aux Persp", flush=True)
            except Exception:
                pass
        except Exception as exc:
            try:
                print(f"[TBS/coupling-trace] UsdNotice install fail: {exc}", flush=True)
            except Exception:
                pass
        ...
        try:
            ...
            for p in notice.GetChangedInfoOnlyPaths():
                ...
                if pos != prev:
                    ...
                    try:
                        print(
                            f"[TBS/coupling-trace] USD_CHANGED label={label!r} "
...
def install_camera_change_tracker(ext: Any) -> None:
    """ext._tbs_coupling_cam_tracker 에 UsdNotice 추적기 설치."""
    if not coupling_diag_enabled():
        return
    try:
        old = getattr(ext, "_tbs_coupling_cam_tracker", None)
        if old is not None:
            try:
                old.teardown()
            except Exception:
        ...
        tracker.install()
        ext._tbs_coupling_cam_tracker = tracker
...
def teardown_camera_change_tracker(ext: Any) -> None:
    try:
        tracker = getattr(ext, "_tbs_coupling_cam_tracker", None)
        if tracker is not None:
            tracker.teardown()
        ext._tbs_coupling_cam_tracker = None
    except Exception:
        pass
...
def log_render_profile_diff(ext: Any) -> None:
    ...
    try:
        ...
        if main_api is None or aux_api is None:
            return
        print("[TBS/coupling-report] -- render-profile-diff --", flush=True)
        ...
        for attr in attrs:
            ...
            try:
                ...
                if mv != av:
                    print(
                        f"[TBS/coupling-report]  DIFF {attr}: main={mv!r} aux={av!r}",
                        flush=True,
                    )
            except Exception as exc:
                print(f"[TBS/coupling-report]  {attr} err={exc}", flush=True)
    except Exception:
        pass
