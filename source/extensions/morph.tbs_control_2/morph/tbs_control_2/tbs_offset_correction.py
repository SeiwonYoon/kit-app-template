...
def apply_world_space_offset_correction(
    ...
    *,
    ...
    Args:
        ...
        asset_tps: (deprecated) 무시됨. LAM 은 FPS 30 고정 정책(TBS_FIXED_FPS)을 따른다.
            호환을 위해 키워드만 남겨두며, 어떤 값을 전달해도 TBS_FIXED_FPS 가 사용된다.
    """
    from .tbs_types import TBS_FIXED_FPS  # 모듈 cycle 회피용 지연 import.
    ...
    try:
        ...
        time_start = Usd.TimeCode(float(start_seconds) * float(TBS_FIXED_FPS))
