...
class AnimationInstanceRuntime:
    ...
    def __init__(
        ...
        # `lam_runtime_evaluator` 가 open 실패 시 `[TBS/Runtime] OPTION_E ...` 를 1 회만 찍기 위함.
    ...
    def setup_offscreen_stage_from_layer(
        ...
        *,
        ...
        try:
            ...
            cur_tcps = -1.0
        if abs(cur_tcps - TBS_FIXED_FPS) > 1e-6:
            try:
                stage.SetTimeCodesPerSecond(TBS_FIXED_FPS)
                stage.SetFramesPerSecond(TBS_FIXED_FPS)
            except Exception:
                pass
        ...
        print(
            ...
            f"layer={ident} timeCode=[{start_tc},{end_tc}]@{TBS_FIXED_FPS}fps "
    ...
    def setup_offscreen_stage(self, asset_path: str) -> bool:
        ...
        if abs(cur_tcps - TBS_FIXED_FPS) > 1e-6:
            try:
                ...
                print(
                    f"{_PRINT_PREFIX} offscreen SetTimeCodesPerSecond({TBS_FIXED_FPS}) FAIL "
        ...
        print(
            ...
            f"source={asset_path} tps={tcps}(forced={TBS_FIXED_FPS}) "
    ...
    def evaluate_and_write(
        ...
        *,
        ...
        Args:
            ...
            snap_timecode_to_frame: True 이면 ``round(vt * TBS_FIXED_FPS)`` 로 정수
        ...
        # FPS 30 고정 — 자산 헤더 / inst.asset_tps 모두 무시하고 TBS_FIXED_FPS 사용.
        tps = TBS_FIXED_FPS
