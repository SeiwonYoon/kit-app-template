...
def _renewal_json_guard_flag(ext: Any, screen: int) -> bool:
    try:
        by = getattr(ext, "_sim_renewal_json_guard_by_screen", None)
        if isinstance(by, dict):
            return bool(by.get(str(int(screen))))
    except Exception:
        pass
    return False


def _active_renewal_job_for_cap(ext: Any, screen: int) -> Optional[Dict[str, Any]]:
    """막대/plan cap 용 — wall busy 와 무관하게 활성 renewal JSON 을 찾는다."""
    scr = int(screen)

    def _job_has_renewal(job: Dict[str, Any]) -> bool:
        if bool(job.get("has_renewal")):
            return True
        try:
    ...
    try:
        ...
        if isinstance(bya, dict):
            ...
            if parallel_moves_enabled():
                ...
                keys.append(str(scr))
            # 화면2 병렬: 레일 중 renewal JSON 이 있으면 그 job 으로 cap
            for k in keys:
                cand = bya.get(k)
                if isinstance(cand, dict) and cand and _job_has_renewal(cand):
                    return dict(cand)
    except Exception:
        pass
    act = _active_gated_event_src(ext, scr)
    if isinstance(act, dict) and act and _job_has_renewal(act):
        return dict(act)
    try:
        by = getattr(ext, "_sim_post_anim_src_by_screen", None)
        if isinstance(by, dict):
            stored = by.get(str(scr))
            if isinstance(stored, dict) and _job_has_renewal(stored):
                return dict(stored)
    except Exception:
        pass
    return None
...
def reset_playback_renewal_runtime(ext: Any, screen: int) -> None:
    ...
    clear_screen_anim_bar_sim_t(ext, scr)
    sk = str(scr)
    try:
        ...
        for rail in rails:
            ...
            if end is None:
                try:
                    ...
                    # wall_sec / event_end 우선 (애니 큐 연장 반영)
                    end_s = str(act.get("event_end_sim_time") or "").strip()
                    if end_s:
                        end = float(end_s)
                    else:
                        wall = float(str(act.get("wall_sec") or "0").strip() or "0")
                        proc = float(str(act.get("proc_sec") or "0").strip() or "0")
                        span = wall if wall > 1e-9 else proc
                        if span > 1e-9:
                            end = float(t0) + float(span)
                except Exception:
                    end = None
...
def _set_screen_anim_bar_sim_t(ext: Any, screen: int, t_sim: float) -> None:
    """해당 화면 JSON 애니가 도달한 sim 시각 — 막대 truncate 상한."""
    try:
        by = getattr(ext, "_sim_anim_bar_sim_t_by_screen", None)
        if not isinstance(by, dict):
            by = {}
            ext._sim_anim_bar_sim_t_by_screen = by
        by[str(int(screen))] = float(max(0.0, float(t_sim)))
    except Exception:
        pass


def clear_screen_anim_bar_sim_t(ext: Any, screen: int) -> None:
    try:
        by = getattr(ext, "_sim_anim_bar_sim_t_by_screen", None)
        if isinstance(by, dict):
...
def _screen_anim_bar_sim_t(ext: Any, screen: int) -> Optional[float]:
    try:
        by = getattr(ext, "_sim_anim_bar_sim_t_by_screen", None)
        if not isinstance(by, dict):
            return None
        v = by.get(str(int(screen)))
        if v is None:
            return None
        return float(v)
    except Exception:
...
def publish_anim_bar_progress_at_step(
    ext: Any,
    screen: int,
    steps: List[Any],
    step_idx: int,
...
def _renewal_plan_lookup_adjust(ext: Any, screen: int, t_lookup: float) -> float:
    ...
    · wall 전 + has_renewal 활성 job: sync 직전 cap
      (``is_json_wall_busy`` 필수 조건 제거 — 화면2 레일/키 누락 시
      cap 이 빠져 막대가 renewal 전에 전진하던 회귀)
    · 스케줄/대기큐 일괄 cap 금지
    """
    t = float(t_lookup)
    scr = int(screen)
    ...
    try:
        act = _active_renewal_job_for_cap(ext, scr)
        if not isinstance(act, dict) or not act:
            return t
...
def _resolve_renewal_sync_t_for_playback(
    ...
    """renewal 포트 sync sim 시각 — 프리런 스케줄·payload SSOT 우선, 없으면 runtime 계산."""
    if not isinstance(src, dict):
        return None
    # 1) 프리런이 job/progress 에 실은 port_sync
    try:
        pst = float(str(src.get("port_sync_sim_time") or "").strip() or "0")
        if pst > 1e-9:
            return float(pst)
    except Exception:
        pass
    # 2) anim_play_start + renewal_offset
    try:
        play0 = float(str(src.get("anim_play_start_sim_time") or "").strip() or "0")
        off = src.get("renewal_offset_sec")
        if play0 > 1e-9 and off is not None and float(off) > 1e-9:
            return float(play0) + float(off)
    except Exception:
        pass
    try:
        t0 = float(
    ...
    try:
        ...
        # SSOT: JSON run start = anim_play_start 우선
        run0 = 0.0
        try:
            run0 = float(str(src.get("anim_play_start_sim_time") or "").strip() or "0")
        except Exception:
            run0 = 0.0
        if run0 <= 1e-9:
            try:
                run0 = float(src.get("_json_run_start_sim") or 0.0)
            except Exception:
                run0 = 0.0
        if run0 > 1e-9 and off is not None and float(off) > 1e-9:
            return float(run0) + float(off)
...
def _resolve_renewal_sync_t_for_wall(
    ...
    """renewal wall 시각 — 프리런 port_sync SSOT 우선 (압축 재계산 금지)."""
    scr = int(screen)
    src_r = dict(src)

    # 1) job/progress 에 실린 프리런 sync (anim_start+offset 포함)
    try:
    ...
    # 2) schedule bake 의 t_playback_port_sync (SSOT 시 프리런과 동일)
    ...
    if step is not None:
        ...
        try:
            ...
            if not bool(SIM_PRERUN_PLAN_SSOT):
                ...
                st2 = renewal_playback_port_sync_for_step(step)
                if st2 is not None and float(st2) > 1e-9:
                    return float(st2)
        except Exception:
            pass
    return None
...
def _playback_ports_at_sim(
    ...
    """재생 포트 SSOT — plan ``ports_at(t)`` 가 기본.

    ``SIM_PRERUN_PLAN_SSOT``: 키프레임만 (renewal delta/hold 금지).
    레거시: renewal 구간 plan + 희소 delta.
    """
    scr = int(screen)
    t = float(t_sim)
    base = _ensure_panel_occ_keys(dict(snap.ports_at(t)))
    ...
    sync_t = _renewal_stored_sync_t(ext, scr)
    hold = _get_renewal_occ_hold(ext, scr)
    sparse = _normalize_sparse_delta(
        hold.get("delta") if isinstance(hold, dict) and isinstance(hold.get("delta"), dict) else None
    )
    if not sparse:
        return base
    if sync_t is not None and float(sync_t) > 1e-9 and t + 1e-6 < float(sync_t):
        return base

    # 레거시: pre_occ + delta
    merged = _renewal_merged_panel_occ(ext, scr, snap, t_sim=t)
...
def _sim_now_for_screen(ext: Any, screen: int, t_sim: Optional[float] = None) -> float:
    """화면 재생 시계. ``t_sim`` 이 주어지면 그 값을 우선 (Seek·renewal explicit)."""
...
def sync_playback_ui_at_sim(ext: Any, screen: int, t_sim: float, *, force: bool = False) -> bool:
    ...
    if (not force) and isinstance(last, dict) and _occ_dicts_equal(dict(last), occ):
        ...
        if isinstance(last_prim, dict) and _occ_dicts_equal(last_prim, prim_occ):
            # occ 불변이어도 헤더 t= 는 sim_now 로 갱신 (키프레임 시각에 고정되는 오해 방지)
            try:
                chans = getattr(ext, "_sim_monitor_channels", None)
                if isinstance(chans, list) and 0 < int(screen) <= len(chans):
                    ch = chans[int(screen) - 1]
                    hdr = (ch or {}).get("port_header") if isinstance(ch, dict) else None
                    if hdr is not None:
                        hdr.text = (
                            f"[포트상태·화면{int(screen)} t={float(axes.t_display):.2f}]"
                        )
            except Exception:
                pass
            return False
...
def refresh_playback_display_at_sim(
    ...
    *,
    ...
    if not bool(getattr(ext, "_sim_playback_started", False)):
        return
    scr = int(screen)
    if bool(explicit) and t_sim is not None:
        try:
            t_use = float(t_sim)
        except Exception:
            t_use = _sim_now_for_screen(ext, scr, None)
    else:
