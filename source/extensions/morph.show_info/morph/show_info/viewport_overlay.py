...
"""3D overlay: panels next to selected prims showing prim info."""
...
def _post_update_once(callback):
    ...
    sub_ref[0] = stream.create_subscription_to_pop(
        _on_event, name="morph.show_info:ViewportOverlayPostUpdateOnce"
    )
    return sub_ref[0]
import omni.ui as ui
from omni.ui import scene as sc

from .prim_info import CHAR_WIDTH, LINE_HEIGHT, PADDING_H, PADDING_V, get_prim_display_lines, get_prim_world_center


# Panel layout
PANEL_OFFSET_SCREEN_PX = 24  # offset panel to the right of prim in screen space
FONT_SIZE_TITLE = 14
FONT_SIZE_BODY = 11
...
CLOSE_BUTTON_OFFSET = 4  # from top-right corner
TITLE_BOTTOM_MARGIN = 10
# 상단 패딩 + X버튼 + 내용 + 하단 패딩 안에 들어가도록 최대 줄 수 (아래로 삐져나옴 방지)
...
class PrimInfoOverlay:
        ...
        with self._viewport_window.get_frame(self._ext_id):
            with ui.ZStack():
                ...
                with self._scene_view.scene:
                    # Root for all panels (no transform; children are at world positions)
    ...
    def _rebuild_panels(self) -> None:
        ...
        try:
            ...
            carb.log_warn(f"[morph.show_info] _rebuild_panels: get_context 실패: {e}")
            return
    ...
    def _build_one_panel(self, world_pos: tuple, path_str: str, lines: List[str]) -> None:
        ...
        if len(lines) > MAX_VISIBLE_LINES:
            # "... N more" 줄까지 포함해 총 MAX_VISIBLE_LINES줄만 표시 (아래로 삐져나옴 방지)
            lines = lines[: MAX_VISIBLE_LINES - 1] + [f"... ({len(lines) - (MAX_VISIBLE_LINES - 1)} more lines)"]
        # 각 줄을 영역 너비에 맞게 잘라서 표시 (overflow 방지)
        ...
        # 패널 높이 = 상단패딩 + X버튼 + 내용줄 + 하단패딩 (아래로 삐져나오지 않도록)
        panel_h = min(
        ...
        close_gesture = sc.ClickGesture(name="morph.show_info:ClosePanel", on_ended_fn=_close)
        ...
        with root:
            with sc.Transform(scale_to=sc.Space.SCREEN):
                with sc.Transform(transform=sc.Matrix44.get_translation_matrix(PANEL_OFFSET_SCREEN_PX, 0, 0)):
                    ...
                    # Content: one label per line (top-down, left-aligned; lines already truncated to fit)
                    content_left = -panel_w // 2 + PADDING_H
                    top_y = panel_h // 2 - PADDING_V - CLOSE_BUTTON_SIZE  # below close button
                    for i, line in enumerate(lines):
                        # 첫 줄(제목)은 그대로, 두 번째 줄부터 제목 아래 여백(TITLE_BOTTOM_MARGIN)만큼 더 아래로
