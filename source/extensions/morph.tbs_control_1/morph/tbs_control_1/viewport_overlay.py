        ...
        with root:
            with sc.Transform(scale_to=sc.Space.SCREEN):
                with sc.Transform(transform=sc.Matrix44.get_translation_matrix(PANEL_OFFSET_SCREEN_PX, 0, 0)):
                    sc.Rectangle(width=panel_w, height=panel_h, color=BG_COLOR, wireframe=False)
                    sc.Rectangle(width=panel_w, height=panel_h, color=BORDER_COLOR, wireframe=True)
                    ...
                    with sc.Transform(transform=sc.Matrix44.get_translation_matrix(btn_x, btn_y, 0)):
                        sc.Rectangle(
                            ...
                            color=X_BUTTON_COLOR, wireframe=False, gesture=close_gesture,
                        )
                    ...
                    for i, line in enumerate(lines):
                        ...
                        with sc.Transform(transform=sc.Matrix44.get_translation_matrix(content_left, y, 0)):
                            ...
                            sc.Label(line, size=size, color=color, alignment=ui.Alignment.LEFT_CENTER)
