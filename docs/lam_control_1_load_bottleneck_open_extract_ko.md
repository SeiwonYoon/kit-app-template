...
**②** Extract가 느린 이유(Master 전체 Flatten 후 장비만 추출)는, 로컬에서 결과를 파일로 만들고 배포에서 그 파일만 읽도록 플래그를 두면 **해결된다.**
로드 후 시뮬 동작은 기존과 같다.
...
아래는 내일 오전 회의에서 **1분 안에 읽으면 되는 말**이다. 기술 용어는 최소만 쓴다.

---

배포에서만 기동이 오래 걸렸습니다. 프로그램이 다른 게 아니라 **실무 USD 규모**와, 기동 시 **로드와 렌더가 겹친 파이프라인** 때문입니다. 병목은 두 구간입니다.

**첫째**는 Master USD를 Kit에 로드하는 구간입니다. 2분할 Viewport와 RTX 렌더가 이미 켜진 상태에서 `open_stage`가 돌아가, **장면을 메모리에 넣는 일과 화면을 그리는 일이 동시에 진행**됐습니다. 이 구간에서 약 20분이 소요됐습니다.  
**개선**은 로드와 Extract가 끝날 때까지 Viewport 갱신·렌더를 일시 정지하고, 완료 후 재개하는 것입니다. Viewport 자체를 나중에 생성하는 구조로 바꾼 것은 아닙니다. 장면 파일을 Kit에 넣는 일 자체는 그대로이므로, 그리기를 멈춘 것만으로 20분이 모두 없어진다고 보기는 어렵습니다. 파일이 Nucleus(원격 서버)에 있으면 네트워크로 읽는 시간이 크고, 같은 파일을 PC에 두고 열면 그 시간은 줄어들 수 있습니다.

**둘째**는 재생용 레이어를 만드는 Extract 구간입니다. 실무 USD는 장비가 각각 다른 파일을 참조하는 구조라, 한 장비만 쓰더라도 **Master 장면 전체의 참조를 풀어 한 레이어로 합치는 작업(Flatten)** 이 먼저 필요합니다. 그다음에 해당 장비만 잘라 붙입니다. 이 Flatten 비용이 첫 장비 Extract에 몰려, 약 30~40분이 걸렸습니다.  
**개선**은 로컬에서 Extract 결과를 파일로 저장해 두고, 배포에서는 Flatten 없이 그 파일만 읽어 붙이는 것입니다. 플래그로 전환합니다. 배포에서는 장면 전체를 다시 Flatten하지 않습니다.
